...
## FRELUX AI API (Phase 7)

The official public API lives at
...
- API keys: 32-char `FLX-…` secrets; only a SHA-256 hash is stored. Raw keys are
  shown exactly once, at create/rotate time.
- API feedback enters the learning pipeline as `USER_PROVIDED` `CANDIDATE` records
  (never auto-approved; structural topics require engineering review).
