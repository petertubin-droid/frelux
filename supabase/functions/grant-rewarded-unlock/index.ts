...
Deno.serve(async (req: Request) => {
  ...
  if (!toolKey || !clientHash) {
    return jsonResponse({ error: 'toolKey and clientHash are required', code: 'BAD_REQUEST' }, 400);
  }
  ...
  if (!toolConfig.is_enabled) {
    return jsonResponse({ error: 'This feature is currently disabled', code: 'DISABLED' }, 403);
  }
  ...
  const dailyLimit = featureConfig?.daily_usage_limit ?? toolConfig.daily_usage_limit ?? 0;
  const cooldownMinutes = featureConfig?.cooldown_minutes ?? toolConfig.cooldown_minutes ?? 0;
  const durationMinutes = featureConfig?.unlock_duration_minutes ?? (toolConfig.unlock_duration_hours * 60) ?? 1440;
  ...
  if (!devMode) {
    ...
    return jsonResponse({
      error: 'No rewarded ad provider is configured for server-side verification.',
      code: 'NO_PROVIDER',
    }, 503);
  }
  ...
  if (cooldownMinutes > 0) {
    const cooldownStart = new Date(Date.now() - cooldownMinutes * 60_000).toISOString();
