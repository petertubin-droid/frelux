...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { email, amount, reference, plan, billing_cycle, user_id, callback_url, metadata } = await req.json();
    ...
    const response = await fetch("https://api.paystack.co/transaction/initialize", {
      ...
      body: JSON.stringify({
        ...
        amount, // in kobo
        reference,
        callback_url: callback_url || `${req.headers.get("origin")}/pricing?status=verify&ref=${reference}`,
        metadata: metadata || {
          plan,
          billing_cycle,
          user_id,
