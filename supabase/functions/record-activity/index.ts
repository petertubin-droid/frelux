...
Deno.serve(async (req: Request) => {
  ...
  if (!activityType) {
    return jsonResponse({ error: 'activityType is required', code: 'BAD_REQUEST' }, 400);
  }
  ...
  const { data: streakData, error: streakError } = await admin.rpc('record_activity', {
    p_user_id: user.id,
  ...
  if (missionTaskType) {
    const { data: missionData, error: missionError } = await admin.rpc('update_mission_progress', {
      p_user_id: user.id,
    ...
    if (missionError) {
      console.error('[record-activity] mission progress error:', missionError.message);
    }
  }
