...
export function createGeminiDevAdapter(opts: {
  ...
  return {
    ...
    async generate(
      ...
      const finishReason = body?.candidates?.[0]?.finishReason as
        string | undefined;
      return {
        parts: parts.map((p) => ({
          text: p.text,
...
// ---------------------------------------------------------
// 6. Provider-agnostic capability engines
//
// Capabilities that need inference (e.g. the ARCHIE CODING
// STUDIO) never reference a provider, provider key or model
// name. They resolve their engine through THIS registry by a
// neutral id, configured via the capability's own env var
// (e.g. ARCHIE_STUDIO_ENGINE). Adding, changing or swapping a
// future engine happens in the registry alone — zero
// consumer redesign. The legacy resolveArchieRuntime above
// remains for systems that already use it.
// ---------------------------------------------------------
