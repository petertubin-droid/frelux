...
export function OnboardingTour({ onComplete }: { onComplete: () => void }) {
  ...
  const [position, setPosition] = useState({
    top: 0,
    left: 0,
    placement: "below" as "below" | "above" | "center",
  });
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-purple/20 bg-card shadow-2xl dark:bg-card">
          ...
          <div className="relative bg-gradient-to-r from-primary to-primary-light px-5 py-4">
            <div className="flex items-start justify-between">
              ...
              <Button
                variant="ghost"
                onClick={skip}
                className="rounded-lg p-1 text-primary-foreground/70 transition-colors hover:bg-white/10 hover:text-primary-foreground"
          ...
          <div className="flex items-center justify-between border-t border-border/50 px-5 py-3 dark:border-white/5">
            ...
            <Button
              variant="default"
              onClick={next}
              className="flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition-all hover:bg-primary/90 press-scale"
            >
              {isLast ? (
                <>
