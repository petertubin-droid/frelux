...
/**
 * Premium result display card used across all calculators.
 * Features: gradient header, animated stats, contextual footer, action buttons.
 */
export default function ResultDisplay({
  ...
  return (
    <div className="mt-8 card overflow-hidden animate-fade-in-up dark:border-white/5">
        ...
        {stats.map((s, i) => (
          <div
            ...
            className={classNames(
              'rounded-xl border p-4 transition-all animate-fade-in-up',
              s.highlight
                ? 'stat-card-highlight'
                : 'stat-card',
            )}
      ...
      <div className="flex flex-col gap-3 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          {continueTo && (
            <Link to={continueTo} state={continueState} className="btn-primary press-scale group">
              {continueLabel}
