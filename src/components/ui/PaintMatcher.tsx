...
export function PaintMatcher() {
  const [status, setStatus] = useState<
    "idle" | "loading" | "matching" | "success" | "error"
  >("idle");
    ...
    <div className="card overflow-hidden">
      <div className="bg-gradient-to-r from-primary to-primary-light p-5 text-primary-foreground">
        ...
        <p className="mt-1 text-xs text-primary-foreground/70">
          Upload a photo of a painted wall and we'll find the closest paint
          color from our library.
        </p>
      </div>
        ...
        {status === "idle" && (
          ...
          >
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              JPG, PNG, or WebP, drag & drop or tap
            </p>
            <input
              ref={fileRef}
              type="file"
            ...
            <div className="relative">
              {preview && (
                <img
                  src={preview}
                  alt="Upload preview"
                  className="h-32 w-32 rounded-xl object-cover opacity-50"
                />
              )}
        ...
        {status === "error" && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <Button
              ...
              type="button"
              onClick={reset}
              className="mt-3 rounded-lg bg-muted px-4 py-2 text-sm font-medium text-muted-foreground dark:bg-card-foreground/90 dark:text-muted-foreground/80"
            >
              Try again
            </Button>
          </div>
          ...
          <div>
            ...
            <div className="mb-4 flex items-center gap-3">
              ...
              <Button
                ...
                type="button"
                onClick={reset}
                className="ml-auto rounded-lg p-2 text-muted-foreground dark:hover:bg-card-foreground/90"
              >
              ...
              {matches.map((match, i) => (
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor: normalizeHex(match.color.hex_code),
                    }}
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {match.color.hex_code.toUpperCase()} · {match.similarity}%
                      match
                    </p>
                  </div>
                  {i === 0 && (
            ...
            <Button
              ...
              type="button"
              onClick={reset}
              className="mt-4 w-full rounded-lg border border-border py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:border-brand-purple hover:text-brand-purple dark:border-border border-border dark:text-muted-foreground/80"
            >
              Match another photo
            </Button>
          </div>
