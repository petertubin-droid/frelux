...
import { FolderPlus, Loader2, Check, X, Folder, ArrowRight, Sparkles } from 'lucide-react';
...
const VALID_TYPES = ['paint', 'screeding', 'pop_ceiling', 'tile', 'finish', 'tyrolene', 'cost', 'build_to_roof', 'structural', 'foundation'];
...
export default function SaveToProjectButton({
  ...
  const mappedType = TYPE_MAP[calculatorType] || (VALID_TYPES.includes(calculatorType) ? calculatorType : 'paint');
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveCalculationToProject({
        ...
        calculator_type: mappedType as any,
  ...
  async function handleCreateAndSave() {
    ...
    try {
      ...
      toast({ title: 'Project created and calculation saved!', type: 'success' });
      setShowModal(false);
  ...
  return (
    <>
      ...
      >
        <FolderPlus className={compact ? 'h-3.5 w-3.5' : 'h-4 w-4 group-hover:rotate-12 transition-transform'} />
        {label}
      </button>
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowModal(false)} className="rounded-lg p-1.5 hover:bg-muted transition-all hover:scale-110">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">Create a new project to save this calculation to.</p>
                <input
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowNewProject(false)} className="flex-1 rounded-lg border px-4 py-2.5 text-sm font-medium hover:bg-muted transition-all">
                    Cancel
                  </button>
                  <button
                  ...
                  >
                    {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><FolderPlus className="h-4 w-4 group-hover:rotate-12 transition-transform" /> Create & Save</>}
                  </button>
                </div>
              </div>
            ) : projects.length === 0 ? (
              <div className="space-y-4">
                <div className="text-center py-8">
                  ...
                  >
                    <FolderPlus className="h-4 w-4 group-hover:rotate-12 transition-transform" /> Create First Project
                  </button>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">Select a project to save this calculation to:</p>
                <div className="max-h-64 space-y-2 overflow-y-auto">
                  {projects.map((proj) => (
                    ...
                    >
                      ...
                      {selectedProject === proj.id && <Check className="h-5 w-5 text-primary" />}
                    </button>
                  ))}
                </div>
                ...
                >
                  <FolderPlus className="h-4 w-4 group-hover:rotate-12 transition-transform" /> Create new project instead
                </button>

                <div className="flex gap-3 pt-2">
                  <button onClick={() => setShowModal(false)} className="flex-1 rounded-lg border px-4 py-2.5 text-sm font-medium hover:bg-muted transition-all">
                    Cancel
                  </button>
                  <button
                    onClick={handleSave}
                    ...
                    className="group flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-primary to-primary/80 px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
                  >
                    {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Check className="h-4 w-4 group-hover:scale-110 transition-transform" /> Save</>}
                  </button>
                </div>
              </div>
            )}
