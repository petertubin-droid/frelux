  ...
  return (
    <div className="flex h-full flex-col">
      <div className="hidden border-b border-border dark:border-white/5 px-5 py-4 lg:block">
        <Link to="/admin/studio" className="flex items-center gap-2">
          <Code className="h-5 w-5 text-brand-purple" />
          <span className="text-base font-bold text-foreground dark:text-primary-foreground">AI Developer Studio</span>
        </Link>
        <p className="mt-0.5 text-xs text-muted-foreground dark:text-muted-foreground">AI assisted development</p>
      </div>

      <nav className="flex-1 overflow-y-auto p-3">
        {TOOL_CATEGORIES.map((cat) => (
          <div key={cat} className="mb-4">
            <p className="mb-1 px-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground dark:text-muted-foreground">{cat}</p>
                ...
                return (
                  <NavLink
                    ...
                    className={({ isActive }) => classNames(
                      ...
                      isActive ? 'bg-primary text-primary-foreground' : 'text-muted-foreground dark:text-muted-foreground/80 hover:bg-muted hover:text-brand-purple'
                    )}
                  >
      ...
      <div className="border-t border-border dark:border-white/5 p-3">
        <Link to="/admin/system-health" className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground dark:text-muted-foreground/80 hover:bg-muted">
          <Activity className="h-4 w-4" /> System Health
        </Link>
        <Link to="/admin" className="mt-1 flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground dark:text-muted-foreground/80 hover:bg-muted">
