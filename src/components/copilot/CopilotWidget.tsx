...
import { Bot, X, Loader2, Send, Sparkles, ShieldCheck, Info, Save } from 'lucide-react';
...
export default function CopilotWidget() {
  ...
  useEffect(() => {
    ...
    fetchContractorProjects()
      .then((data) => setProjects(data.map((p) => ({ id: p.id, name: p.name })).slice(0, 12)))
      .catch(() => undefined);
  }, [open, user, projects.length]);

  const buildContext = useCallback((): FreluxContext => ({
    userId: user?.id ?? null,
    project: null,
    location: null,
    calculations: [],
  ...
  const handleSubmit = useCallback(async (text?: string) => {
    ...
    setPlan(planTask(interpretation.taskType, buildContext(), interpretation.facts));
    setPhase('review');
    setBusy(false);
  ...
  const handleRunEngine = useCallback(async () => {
    ...
    const result = await runTask(
      ...
      buildContext(),
      [...interpretation.facts, ...suppliedFacts],
    );
    setOutcome(result);
  ...
  return (
    <div
      className="fixed bottom-20 left-1/2 z-50 flex h-[min(78vh,640px)] w-[calc(100vw-2rem)] max-w-sm -translate-x-1/2 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl dark:border-white/5 sm:bottom-4 sm:left-4 sm:translate-x-0"
      role="dialog"
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setOpen(false); reset(); }} aria-label="Close Copilot">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3" ref={resultRef}>
        ...
        {phase === 'result' && outcome?.result && (
          <ResultPanel
            ...
            saveState={saveState}
            onSave={handleSave}
            onOpenEstimator={() => { setOpen(false); navigate('/build-to-roof-estimator'); }}
            onNewRequest={reset}
          />
        )}
        ...
        {phase === 'refused' && outcome?.refusal && (
          <div className="space-y-3 py-4">
            ...
            <Button size="sm" variant="outline" onClick={reset}>Try another request</Button>
          </div>
        )}
      </div>
      ...
      >
        <input
          ...
          maxLength={400}
        />
        <Button type="submit" size="icon" disabled={busy || !draft.trim()} aria-label="Send request">
...
function ReviewPanel({
  ...
  const missing = plan?.steps.find((s) => s.kind === 'request_missing_info')?.missingFields ?? [];
  const refused = plan?.steps.find((s) => s.kind === 'refuse');

  return (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Understood task</p>
      ...
      {interpretation.facts.length > 0 && (
        <div className="space-y-1.5">
          {interpretation.facts.map((fact) => (
            <div key={fact.key + String(fact.value)} className="flex items-start justify-between gap-2 rounded-md border border-border px-3 py-2 text-sm">
              <span>
                <span className="font-medium">{fact.label}:</span> {String(fact.value)}{fact.unit ? ` ${fact.unit}` : ''}
                {fact.evidence && <span className="block text-[11px] text-muted-foreground">{fact.evidence}</span>}
              </span>
              <span className={classNames('shrink-0 rounded-full px-2 py-0.5 text-[10px]', badgeClass(fact))}>
                {trustBadge(fact)}
              </span>
            </div>
          ))}
          ...
          {missing.map((field) => (
            <label key={field.key} className="block text-sm">
              ...
              <input
                ...
                onChange={(e) => setMissingInput({ ...missingInput, [field.key]: e.target.value })}
                inputMode={field.unit === 'm' || field.unit === 'count' ? 'decimal' : undefined}
  ...
  return (
    <div className="space-y-4">
      ...
      {result.quantities.length > 0 && (
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Key quantities</p>
          <ul className="mt-1 space-y-1 text-sm">
            {result.quantities.slice(0, 6).map((q, i) => (
              <li key={`${q.label}-${i}`} className="flex justify-between">
                <span className="text-muted-foreground">{q.label}</span>
                <span className="font-medium">{q.quantity.toLocaleString()} {q.unit}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {outcome.assumptions && outcome.assumptions.length > 0 && (
        <div className="rounded-lg border border-border p-3 text-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Assumptions used (editable)</p>
          <ul className="mt-1 space-y-1 text-xs text-muted-foreground">
            {outcome.assumptions.map((a) => (
              <li key={a.key}>• {a.label}: {String(a.value)}{a.unit ? ` ${a.unit}` : ''}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className="space-y-2">
        <Button size="sm" variant="outline" className="w-full" onClick={onOpenEstimator}>
          Open full estimator breakdown
        </Button>
        {projects.length > 0 && saveState !== 'saved' && (
          <div className="space-y-1">
            <label className="block text-xs text-muted-foreground">Save this estimate to a project (your confirmation):</label>
            <select
        ...
        {saveState === 'saved' && (
          <p className="flex items-center gap-1 text-xs text-emerald-600"><Save className="h-3 w-3" aria-hidden /> Saved to your project.</p>
        )}
        <Button size="sm" variant="ghost" className="w-full" onClick={onNewRequest}>New request</Button>
      </div>
    </div>
  );
}
...
function badgeClass(fact: AiFact): string {
  if (fact.trust === 'user_confirmed' || fact.trust === 'corrected') return 'bg-emerald-500/15 text-emerald-700';
  if (fact.origin === 'user_input' || fact.origin === 'engine_calculation') return 'bg-emerald-500/15 text-emerald-700';
