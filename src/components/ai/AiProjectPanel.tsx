...
export function AiProjectPanel({ projectData }: AiProjectPanelProps) {
  ...
  const actions: {
  ...
  }[] = [
    {
      ...
      label: "Review",
      icon: Brain,
      desc: "Full project analysis, mistake detection, missing materials",
    },
    {
      key: "optimize",
      label: "Cost Optimizer",
      icon: TrendingDown,
      desc: "Reduce costs, create premium/economy/luxury versions",
    },
    {
      key: "explain",
      label: "Explain",
      icon: BookOpen,
      desc: "Step-by-step breakdown of every calculation",
    },
    {
      key: "qa",
      label: "Ask AI",
      icon: HelpCircle,
      desc: "Technical construction questions and guidance",
    },
  ];
        ...
        <div className="mt-4 space-y-4">
          ...
          {parsed.roomBreakdowns && (
            <div>
              ...
              {parsed.roomBreakdowns.map(
                (
                  room: { roomName: string; calculations: string[] },
                  i: number,
                ) => (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Get intelligent insights, cost optimizations, and expert guidance
          powered by AI.
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && question.trim()) runAi();
              }}
      ...
      {(activeAction === "review" || activeAction === "explain") && (
        <Button
          ...
          onClick={runAi}
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-lg px-6 py-2.5 text-sm font-medium transition-colors hover:bg-primary/90 disabled:opacity-50"
        >
          {loading ? (
            <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" />
          ) : (
