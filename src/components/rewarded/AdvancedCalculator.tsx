        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-100">
            {estimate.lineItems.map((item) => (
              <tr key={item.label}>
                <td className="px-3 py-2.5 font-medium text-brand-navy">{item.label}</td>
                ...
                <td className="px-3 py-2.5 text-right font-semibold text-brand-navy">{formatCurrency(item.total, estimate.currencySymbol)}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function MixTab({ input, update, config }: {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h4 className="text-sm font-bold text-brand-navy">Custom Mix Ratio Editor</h4>
          ...
          {ratios.map((r) => (
            <button
              ...
              className={
                ...
                (input.mixRatio === r ? 'border-brand-purple bg-brand-purple text-white' : 'border-neutral-200 text-neutral-600 hover:border-neutral-300')
              }
            >
              {r}
            </button>
...
function CostsTab({ input, update, estimate }: {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h4 className="text-sm font-bold text-brand-navy">Waste Percentage Scenarios</h4>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {[0, 5, 10, 15, 20, 25].map((w) => (
            <button
              ...
              className={
                'rounded-lg border px-4 py-2 text-sm font-semibold transition-all ' +
                (input.wastePercentage === w ? 'border-brand-purple bg-brand-purple text-white' : 'border-neutral-200 text-neutral-600 hover:border-neutral-300')
              }
            >
              {w}%
            </button>
      ...
      <div className="rounded-lg border border-neutral-200 p-4">
        <h4 className="text-sm font-bold text-brand-navy">Cost Summary</h4>
        <div className="mt-3 space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between border-t border-neutral-200 pt-2">
            ...
            <span className="text-lg font-bold text-brand-navy">{formatCurrency(estimate.grandTotal, estimate.currencySymbol)}</span>
          </div>
        </div>
      </div>
    </div>
...
function CompareTab({ current, saved, onSelect, selected }: {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h4 className="text-sm font-bold text-brand-navy">Cost Comparison</h4>
            ...
            {saved.map((s) => (
              <button
                ...
                className={
                  ...
                  (selected === s.estimateData ? 'border-brand-purple bg-brand-purple text-white' : 'border-neutral-200 text-neutral-600 hover:border-neutral-300')
                }
              >
                {s.title}
              </button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-neutral-100">
              {[
                ...
                return (
                  <tr key={row.label}>
                    <td className="px-3 py-2 font-medium text-brand-navy">{row.label}</td>
...
function AiTab({ question, setQuestion, onAsk, onRecommend, loading, response }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h4 className="text-sm font-bold text-brand-navy">Ask the AI Assistant</h4>
