...
export function RewardedFeatureGate({ toolKey, featureName, features, children }: Props) {
  ...
  if (access.loading) {
    return (
      <div className="mt-6 flex items-center justify-center gap-2 rounded-xl border border-neutral-200 bg-neutral-50 py-6 text-sm text-neutral-400 dark:border-white/5 dark:bg-white/5 dark:text-neutral-500 dark:border-white/5 dark:bg-white/5 dark:text-neutral-500">
        <div className="h-4 w-4 animate-spin rounded-full border-2 border-neutral-200 border-t-brand-purple dark:border-white/10 dark:border-t-brand-purple-lighter dark:border-white/10 dark:border-t-brand-purple-lighter" />
        Loading…
      </div>
    );
  }
    ...
    <>
      <div className="mt-6 overflow-hidden rounded-2xl border border-brand-purple/20 bg-gradient-to-br from-brand-purple/5 to-transparent">
        <div className="p-6 text-center">
          ...
          <h3 className="mt-3 text-base font-bold text-brand-navy dark:text-white dark:text-white">{featureName}</h3>
