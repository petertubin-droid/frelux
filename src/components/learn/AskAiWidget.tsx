...
export default function AskAiWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-30 flex h-[500px] max-h-[80vh] w-[380px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-border dark:border-white/5 bg-card dark:bg-card shadow-2xl">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border dark:border-white/5 bg-background px-4 py-3">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <MessageSquare className="h-10 w-10 text-muted-foreground/80" />
            <p className="mt-3 text-sm font-semibold text-muted-foreground dark:text-muted-foreground/80">Ask me anything about painting</p>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={classNames('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                <div className={classNames(
                  ...
                  msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted text-card-foreground dark:text-muted-foreground/60'
                )}>
