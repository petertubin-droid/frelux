      ...
      {loading ? (
        ...
        <article>
          <header className="mt-6 border-b border-neutral-200 pb-6">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-brand-navy sm:text-4xl">{page!.title}</h1>
        ...
        <>
          <header className="mt-6 border-b border-neutral-200 pb-6">
            <p className="section-label mb-2">Legal</p>
            <h1 className="text-3xl font-bold tracking-tight text-brand-navy sm:text-4xl">{title}</h1>
            {updated && <p className="mt-2 text-sm text-neutral-400">Draft · last updated {updated}</p>}
            {intro && <div className="mt-4 text-sm text-neutral-600">{intro}</div>}
          </header>

          <div className="mt-8 space-y-8">
            {sections.map((s) => (
              <section key={s.heading}>
                <h2 className="text-lg font-bold text-brand-navy">{s.heading}</h2>
                <div className="mt-2 text-sm leading-relaxed text-neutral-600">{s.body}</div>
              </section>
            ))}
          </div>

          <div className="mt-12 rounded-lg border border-dashed border-neutral-200 bg-neutral-50 p-4 text-xs text-neutral-400">
