...
function slugifyProfessionals(categorySlug: string): string {
  ...
  return map[categorySlug] || categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1);
}
...
export default function ProCategoryLocationPage() {
  ...
  useSeo({
    ...
    structuredData: breadcrumbSchema([
      ...
      { name: locationLabel || 'Location', path: `/pro/${categorySlug}/${locationSlug}` },
    ]),
  });
      ...
      <div className="border-b border-neutral-200 bg-white dark:border-white/5 dark:bg-brand-navy-mid">
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400">
            ...
            <span>/</span>
            <span className="text-neutral-900 dark:text-white">{professionLabel} in {location?.city || '...'}</span>
          </nav>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        {loading ? (
          ...
          <div className="rounded-xl border border-neutral-200/60 bg-white p-8 text-center dark:border-white/5 dark:bg-brand-navy-mid">
            ...
            <p className="mt-2 text-sm text-neutral-500">No {professionLabel.toLowerCase()} listed in {locationLabel} yet.</p>
            <Link to="/pro-connect/register" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-brand-purple px-4 py-2 text-sm font-semibold text-white">
              Register as a Pro
            </Link>
          </div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1.5 flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
                        ...
                        {pro.years_experience && <span>· {pro.years_experience} yrs exp</span>}
                      </div>
                    </div>
                  </div>
                </Link>
