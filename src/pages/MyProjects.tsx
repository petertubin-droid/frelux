      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {tabs.map((t) => {
            const Icon = t.icon;
            return (
              <button key={t.id} type="button" onClick={() => { setTab(t.id); setSearch(''); }} className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-semibold transition-all ${tab === t.id ? 'border-brand-purple bg-brand-purple text-white' : 'border-neutral-200 bg-white text-neutral-600 hover:text-brand-purple'}`}>
              ...
              {filteredProjects.map((p) => (
                <div key={p.id} className="flex items-center justify-between rounded-xl border border-neutral-200 bg-white p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-brand-navy">{p.name}</h3>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {favPalettes.map((p) => (
                <Link key={p.id} to={`/colors/${p.slug}`} className="group overflow-hidden rounded-xl border border-neutral-200 bg-white transition-all hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div className="p-4"><h3 className="text-base font-bold text-brand-navy">{p.title}</h3></div>
                </Link>
              ))}
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        {renamingId === c.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => setRenamingId(null)} className="rounded-md border border-neutral-200 p-1.5 text-neutral-500"><X className="h-4 w-4" /></button>
                          </div>
                        ) : (
                          <>
                      ...
                      {renamingId !== c.id && (
                        <div className="flex shrink-0 items-center gap-2">
                          <button type="button" onClick={() => { setRenamingId(c.id); setRenameValue(c.name); }} className="rounded-lg border border-neutral-200 p-2 text-neutral-500 hover:text-brand-purple" title="Rename"><Pencil className="h-4 w-4" /></button>
                        ...
                        {(collectionColors[c.id] ?? []).map((col) => (
                          <div key={col.id} className="group relative flex items-center gap-2 rounded-lg border border-neutral-200 p-1.5 pr-3 hover:border-brand-purple">
                            <Link to={`/colors/paint/${col.slug}`} className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-medium text-neutral-700">{col.name}</span>
                            </Link>
                            {collections.length > 1 && (
                              <div className="relative">
                                ...
                                {moveState?.colorId === col.id && (
                                  <div className="absolute right-0 top-6 z-10 w-48 rounded-lg border border-neutral-200 bg-white p-2 shadow-lg">
                                    <p className="mb-1 text-[10px] font-semibold uppercase text-neutral-400">Move to</p>
                                    {collections.filter((oc) => oc.id !== c.id).map((oc) => (
                                      <button key={oc.id} type="button" onClick={() => handleMoveColor(oc.id)} className="block w-full rounded px-2 py-1.5 text-left text-xs font-medium text-neutral-700 hover:bg-neutral-100">{oc.name}</button>
                                    ))}
                                  </div>
                                )}
                              </div>
            ...
            {recentlyViewed.length > 0 ? (
              <>
                <div className="mb-4 flex justify-end">
                  <button type="button" onClick={handleClearRecent} className="inline-flex items-center gap-2 rounded-lg border border-neutral-200 px-3 py-1.5 text-sm font-medium text-neutral-600 hover:text-red-500">
                    <Trash2 className="h-4 w-4" /> Clear History
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                  {recentlyViewed.map((c) => (
                    <div key={c.id} className="group relative overflow-hidden rounded-lg border border-neutral-200 bg-white transition-all hover:-translate-y-1 hover:shadow-md">
                      <button type="button" onClick={() => handlePinRecent(c.id)} className="absolute right-2 top-2 z-10 rounded-full bg-white/80 p-1.5 text-neutral-500 hover:text-brand-purple" title="Pin/Unpin">
