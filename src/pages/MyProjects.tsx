...
import {
  ...
  moveColorToCollection,
  fetchRecentlyViewedColors,
  clearRecentlyViewed,
  togglePinRecentlyViewed,
  createShareableLink,
} from "@/lib/queries";
...
export default function MyProjects() {
  useSeo({
    ...
    description:
      "View your saved paint colors, color palettes, calculations, and custom collections.",
  ...
  async function loadAll() {
    setStatus("loading");
    setError("");
    const [projRes, favColRes, favPalRes, collRes, recentRes] =
      await Promise.all([
        fetchUserProjects(),
        fetchFavoriteColors(),
        fetchFavoritePalettes(),
  ...
  async function handleMoveColor(toCollectionId: string) {
    if (!moveState) return;
    const { error } = await moveColorToCollection(
      moveState.fromColl,
      toCollectionId,
      moveState.colorId,
    );
    ...
    return (
      <>
        <PageHeader
          ...
          title="My Projects"
          subtitle="Your saved colors, palettes, calculations, and custom collections, all in one place."
  ...
  const tabs: { id: Tab; label: string; icon: typeof Folder; count: number }[] =
    [
      ...
      },
      {
        ...
        icon: Bookmark,
        count: favColors.length,
      },
      {
        id: "palettes",
        label: "Favorite Palettes",
        icon: PaletteIcon,
        count: favPalettes.length,
      },
      {
        id: "collections",
        label: "Collections",
          ...
          <>
            <div className="mb-6 flex gap-2">
              <input
                ...
                placeholder="New collection name (e.g., Luxury Living Room)"
