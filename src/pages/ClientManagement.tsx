    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {showForm && (
        <form
          ...
          className="mt-4 space-y-4 rounded-lg border p-6"
        >
    ...
    <div className="rounded-lg border p-4 transition-colors hover:border-brand-purple/30">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-1">
          <Button variant="ghost" onClick={onEdit} className="rounded p-1.5">
            <Edit aria-hidden="true" className="h-4 w-4" />
          </Button>
          <Button variant="ghost" onClick={onDelete} className="rounded p-1.5">
      ...
      <div className="mt-3 space-y-1">
        {client.email && (
          <p className="flex items-center gap-2 text-xs text-muted-foreground">
            <Mail aria-hidden="true" className="h-3 w-3" /> {client.email}
          </p>
        )}
        {client.phone && (
          <p className="flex items-center gap-2 text-xs text-muted-foreground">
            <Phone aria-hidden="true" className="h-3 w-3" /> {client.phone}
          </p>
        )}
        {client.address && (
          <p className="flex items-center gap-2 text-xs text-muted-foreground">
            <MapPin aria-hidden="true" className="h-3 w-3" /> {client.address}
          </p>
        )}
      </div>
      {client.notes && (
...
function ClientDetailDrawer({
  ...
  const [commForm, setCommForm] = useState({
    type: "note" as DbClientCommunication["type"],
    subject: "",
    body: "",
  });
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-end bg-black/50 sm:items-center sm:justify-center"
      onClick={onClose}
    >
      <div
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-card p-6 sm:rounded-2xl"
        ...
        <div className="mt-4 space-y-1">
          {client.email && (
            <p className="flex items-center gap-2 text-sm text-muted-foreground">
              <Mail aria-hidden="true" className="h-4 w-4" /> {client.email}
            </p>
          )}
          {client.phone && (
            <p className="flex items-center gap-2 text-sm text-muted-foreground">
              <Phone aria-hidden="true" className="h-4 w-4" /> {client.phone}
            </p>
          )}
          {client.address && (
            <p className="flex items-center gap-2 text-sm text-muted-foreground">
              <MapPin aria-hidden="true" className="h-4 w-4" /> {client.address}
            </p>
          )}
          {client.notes && (
            <p className="mt-3 rounded-lg border p-3 text-sm">{client.notes}</p>
          )}
        </div>
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium">
                      {p.currency_symbol}
                      {(p.total_project_cost ?? 0).toLocaleString()}
                    </p>
                    <p className={`text-xs ${statusColors[p.status] ?? ""}`}>
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setCommForm({
                    ...commForm,
                    type: e.target.value as DbClientCommunication["type"],
                  })
                }
              ...
              <textarea
                ...
                value={commForm.body}
                onChange={(e) =>
                  setCommForm({ ...commForm, body: e.target.value })
                }
                placeholder="Details..."
                className="w-full rounded-lg border bg-background px-3 py-2 text-sm"
              />
