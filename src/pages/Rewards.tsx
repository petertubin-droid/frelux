...
export default function Rewards() {
  useSeo({
    ...
    description: 'Earn FRELUX Credits by using the platform, complete weekly missions, maintain activity streaks, and redeem rewards.',
  ...
  async function handleRedeem(reward: RewardItem) {
    ...
    if ((wallet?.balance ?? 0) < reward.credit_cost) {
      toast({ type: 'warning', title: 'Not enough credits', message: `You need ${reward.credit_cost} credits to redeem this.` });
      return;
    }
    ...
    if (result.success) {
      ...
      if (result.error === 'already_redeemed') {
        toast({ type: 'info', title: 'Already redeemed', message: 'This reward was already processed.' });
      } else if (result.error === 'insufficient_credits') {
        toast({ type: 'warning', title: 'Insufficient credits', message: 'You don\'t have enough credits.' });
      } else {
        toast({ type: 'error', title: 'Redemption failed', message: result.error ?? 'Please try again.' });
      }
    }
    setRedeeming(null);
  }
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold text-brand-navy dark:text-white">FRELUX Rewards</h1>
        <p className="mt-2 text-sm text-neutral-500">Sign in to start earning credits and unlocking rewards.</p>
        <Link
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 rounded-lg bg-brand-purple/10 px-2.5 py-1.5">
                    ...
                    <span className="text-sm font-bold text-brand-purple">{reward.credit_cost}</span>
                  </div>
                </div>
                <button
...
// Weekly Mission Card
// =========================================================
function WeeklyMissionCard({ mission, progress }: { mission: WeeklyMission; progress: MissionProgress[] }) {
  const tasks = mission.mission_config.tasks ?? [];
  const completedCount = tasks.filter(t => progress.find(p => p.task_key === t.key && p.completed)).length;
    ...
    <div className="mb-6 rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-transparent p-6 dark:border-blue-500/20 dark:from-blue-500/5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-brand-navy dark:text-white">This Week's Mission</h3>
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-blue-100 px-2.5 py-1 dark:bg-blue-500/10">
          ...
          <span className="text-sm font-bold text-blue-600 dark:text-blue-400">+{mission.mission_config.reward_credits}</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {tasks.map((task) => {
          ...
          return (
            <div key={task.key} className="flex items-center gap-3 rounded-lg border border-blue-100 bg-white p-3 dark:border-blue-500/10 dark:bg-white/5">
              <div className={classNames(
                'flex h-6 w-6 items-center justify-center rounded-full',
                done ? 'bg-blue-500 text-white' : 'bg-blue-50 text-blue-400 dark:bg-blue-500/10'
              )}>
...
// Transaction History
// =========================================================
function TransactionHistory({ transactions }: { transactions: CreditTransaction[] }) {
  if (transactions.length === 0) {
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-8 text-center dark:border-white/5 dark:bg-brand-navy-mid">
        ...
        <p className="mt-3 text-sm text-neutral-500">No transactions yet. Start earning credits!</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-neutral-200 dark:border-white/5">
      <div className="divide-y divide-neutral-100 dark:divide-white/5">
        {transactions.map((tx) => (
          <div key={tx.id} className="flex items-center gap-3 bg-white px-4 py-3 dark:bg-brand-navy-mid">
            <div className={classNames(
