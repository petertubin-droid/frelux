...
export default function TileCalculator({
  ...
  useSeo(
    !embedded
      ? {
          ...
          description:
            "Free tile calculator. Enter your floor or wall dimensions and tile size to calculate tile quantity, boxes, adhesive, grout, and labour cost.",
          canonicalPath: "/tile-calculator",
          ogType: "website",
          keywords:
            "tile calculator, how many tiles do i need, tile quantity calculator, floor tile calculator, wall tile calculator",
          structuredDataArray: [
            {
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: "Measure the area to be tiled and divide by the area of one tile. Add 10–15% for cuts and breakage.",
                  },
                },
                {
                  "@type": "Question",
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: "Buy 10–15% more tiles than calculated to cover cuts, breakage, and future repairs.",
                  },
                },
              ],
            },
  ...
  useEffect(() => {
    trackRecentTool("/tile-calculator", "Tile Calculator", "Grid3x3");
    return () => {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (labour) {
        setInput((prev) => ({
          ...prev,
          labourRatePerSqm: Number(labour.labour_rate_per_sqm),
        }));
      }
  ...
  function update<K extends keyof TileCalcInput>(
    key: K,
    value: TileCalcInput[K],
  ) {
  ...
  function compute() {
    ...
    if (input.surfaceType === "floor" && input.width <= 0)
      e.width = "Enter a valid width";
    if (input.surfaceType === "wall" && input.height <= 0)
      e.height = "Enter a valid height";
    setErrors(e);
    if (Object.keys(e).length > 0) return;

    const r = monitoredCalc("Tile Calculator", () =>
      calculateTile(input, tileMaterials, currency, currencySymbol),
    );
    trackCalculation("tile");
    trackCalculationWithRewards("tile", "Tile Calculator");
    setResult(r);
    track("tile_calculated", {
      surfaceType: input.surfaceType,
      area: r.surfaceArea,
      tiles: r.tilesNeeded,
    });
    void saveEstimateHistory(user?.id ?? null, {
      calculator_type: "tile",
      project_name: `Tile: ${input.surfaceType}`,
      input_data: input as unknown as Record<string, unknown>,
      result_data: r as unknown as Record<string, unknown>,
    }).catch(() => {});
    logAnalyticsEvent("tile_calculated", {
      surfaceType: input.surfaceType,
      area: r.surfaceArea,
      tiles: r.tilesNeeded,
    });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const { error } = await saveUserProject("Tile Calculation", "tile", {
      ...input,
      result,
    });
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <LoadTemplateButton
          calculatorType="tile"
          onLoad={(t) => setInput(t.input_data as unknown as TileCalcInput)}
        />
        <SaveTemplateButton
          calculatorType="tile"
          inputData={input as unknown as Record<string, unknown>}
          defaultName={`${input.length}×${input.width} ${input.surfaceType}`}
        />
      </div>
      <div
        role="region"
        aria-label="Tile calculator"
        ...
        {!result && (
          <div className="calc-card card p-6 sm:p-8 dark:border-white/5 dark:bg-card">
            ...
            <div className="mt-6">
              <Field
                label="Tile size"
                hint="Select a standard size or enter custom dimensions"
              >
                <select
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {(
                  [
                    {
                      value: "adhesive",
                      label: "Tile Adhesive Method",
                      desc: "Pre-mixed adhesive (recommended)",
                    },
                    {
                      value: "traditional",
                      label: "Traditional Method",
                      desc: "Cement + sharp sand mix",
                    },
                  ] as const
                ).map((m) => {
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <Field label={`Adhesive price per bag (${currencySymbol})`}>
                  <input
                    type="number"
                    min={0}
                    value={input.adhesivePricePerBag || ""}
                    onChange={(e) =>
                ...
                <Field label="Adhesive coverage (m²/bag)">
                  <input
                    ...
                    min={0}
                    step="0.1"
                    value={input.adhesiveCoverageRate || ""}
                    onChange={(e) =>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <Field label={`Tile spacers price per pack (${currencySymbol})`}>
                <input
                  type="number"
                  min={0}
                  value={input.spacerPricePerPack || ""}
                  onChange={(e) =>
                    update("spacerPricePerPack", Number(e.target.value))
                  }
                  className="input-field"
                  placeholder="0"
                />
              ...
              <Field label="Spacer coverage (m²/pack)">
                <input
                  ...
                  min={0}
                  step="0.1"
                  value={input.spacerCoverageRate || ""}
                  onChange={(e) =>
                    update("spacerCoverageRate", Number(e.target.value))
                  }
                  className="input-field"
                  placeholder="0"
                />
        ...
        {result && (
          <TileResultCard
            ...
            calcDefaults={{
              ...
              estimateDisclaimer: calcDefaults.estimateDisclaimer,
            }}
            engine={engine}
  ...
  return (
    <div className="mt-8 card overflow-hidden dark:border-white/5">
      ...
      <div className="grid gap-4 p-6 sm:grid-cols-2 sm:p-8 dark:bg-card">
        ...
        <Stat
          label="Tile area"
          value={`${formatNumber(result.tileArea, 3)} m²`}
        />
        <Stat label="Tiles needed" value={`${result.tilesNeeded}`} />
        <Stat label="Boxes needed" value={`${result.boxesNeeded}`} />
        {result.method === "adhesive" && (
          <Stat
            label="Adhesive needed"
            value={`${result.adhesiveNeeded} bag(s)`}
          />
        )}
        {result.method === "traditional" && (
          <Stat label="Cement needed" value={`${result.cementNeeded} bag(s)`} />
        )}
        {result.method === "traditional" && (
          <Stat
            label="Sharp sand needed"
            value={`${result.sandNeeded} bag(s)`}
          />
        )}
        <Stat label="Grout needed" value={`${result.groutNeeded} kg`} />
        <Stat
          label="Tile spacers needed"
          value={`${result.spacerNeeded} pack(s)`}
        />
      </div>
      ...
      <div className="mt-2 space-y-2 px-6 pb-6 sm:px-8">
        <Row
          label="Tile cost"
          value={formatCurrency(result.tileCost, currencySymbol)}
        />
        {result.method === "adhesive" && (
          <Row
            label="Adhesive cost"
            value={formatCurrency(result.adhesiveCost, currencySymbol)}
          />
        )}
        {result.method === "traditional" && (
          <Row
            label="Cement cost"
            value={formatCurrency(result.cementCost, currencySymbol)}
          />
        )}
        {result.method === "traditional" && (
          <Row
            label="Sharp sand cost"
            value={formatCurrency(result.sandCost, currencySymbol)}
          />
        )}
        <Row
          label="Grout cost"
        ...
        />
        <Row
          label="Tile spacers cost"
          value={formatCurrency(result.spacerCost, currencySymbol)}
        />
        <Row
          label="Labour cost"
      ...
      {saveMsg && (
        <p className="px-6 pb-2 text-sm text-brand-purple sm:px-8">{saveMsg}</p>
      )}
        ...
        <EngineExplanationPanel
          result={engine.buildExplanation({
            ...
            steps: [
              ...
              {
                description: "Tile area",
                value: `${formatNumber(result.tileArea, 3)} m²`,
              },
              {
                description: "Tiles needed",
                value: String(result.tilesNeeded),
              },
              {
                description: "Boxes needed",
                value: String(result.boxesNeeded),
              },
              {
                description: "Waste amount",
                value: `${result.wasteAmount} tiles`,
              },
              ...(result.method === "adhesive"
                ? [
                    {
                      description: "Adhesive needed",
                      value: `${result.adhesiveNeeded} bags`,
                    },
                  ]
                : []),
              ...(result.method === "traditional"
                ? [
                    {
                      description: "Cement needed",
                      value: `${result.cementNeeded} bags`,
                    },
                  ]
                : []),
              {
                description: "Grout needed",
          ...
          summary={engine.buildMaterialSummary([
            {
              materialId: "tiles",
              productName: "Tiles",
              totalQuantity: result.boxesNeeded,
              quantityUnit: "boxes",
              spaceIds: ["surface"],
            },
            ...(result.method === "adhesive"
              ? [
                  {
                    ...
                    totalQuantity: result.adhesiveNeeded,
                    quantityUnit: "bags",
                    spaceIds: ["surface"],
                  },
                ]
              ...
              ? [
                  {
                    materialId: "cement",
                    productName: "Cement",
                    totalQuantity: result.cementNeeded,
                    quantityUnit: "bags",
                    spaceIds: ["surface"],
                  },
                ]
              : []),
            {
              materialId: "grout",
              productName: "Grout",
              totalQuantity: result.groutNeeded,
              quantityUnit: "kg",
              spaceIds: ["surface"],
            },
          ])}
      ...
      <HowCalculatedSection
        ...
        assumptions={[
          ...
          {
            label: "Tile size",
            value: `${input.tileWidthMm}×${input.tileHeightMm}mm`,
          },
