...
import { ArrowLeft, Copy, Check, AlertCircle, Loader2, Heart, ArrowRight } from 'lucide-react';
import { fetchPaintColorBySlug, fetchRelatedPaintColors, fetchColorFamilies, fetchColorCategories, trackColorView, toggleFavoriteColor, fetchFavoriteColorIds, fetchRelationshipOverrides, fetchPaintColors } from '@/lib/queries';
import { useSeo } from '@/lib/seo';
import { useAuth } from '@/lib/auth';
import { readableTextColor, complementaryColor, analogousColors, triadicColors, lighterColor, darkerColor, matchingTrimColor, matchingCeilingColor, coordinatedAccentColor, normalizeHex } from '@/lib/colors';
import { classNames } from '@/lib/utils';
import type { DbPaintColor, DbColorFamily, DbColorCategory, DbColorRelationshipOverride, ColorRelationshipType } from '@/types/database';
...
export default function PaintColorDetail() {
  ...
  useSeo({
    ...
    structuredData: color ? {
      ...
      category: families.find((f) => f.id === color.color_family_id)?.name ?? 'Paint Color',
      brand: { '@type': 'Brand', name: 'FRELUX' },
      additionalProperty: [
        { '@type': 'PropertyValue', name: 'HEX', value: color.hex_code },
        { '@type': 'PropertyValue', name: 'RGB', value: `rgb(${color.rgb_r}, ${color.rgb_g}, ${color.rgb_b})` },
        { '@type': 'PropertyValue', name: 'HSL', value: `hsl(${Math.round(color.hsl_h)}, ${Math.round(color.hsl_s)}%, ${Math.round(color.hsl_l)}%)` },
      ],
    } : undefined,
  });
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!data) { setStatus('ready'); setColor(null); return; }
      setColor(data);

      const [relRes, famRes, catRes, overrideRes, allColorsRes] = await Promise.all([
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-2">
        ...
        <div className="overflow-hidden rounded-2xl border border-neutral-200 shadow-sm">
          <div className="flex aspect-square items-center justify-center" style={{ background: color.hex_code }}>
            <span className="text-3xl font-bold uppercase tracking-widest" style={{ color: textColor }}>{color.hex_code}</span>
          </div>
          {user && (
            <button type="button" onClick={handleFav} className={classNames('flex w-full items-center justify-center gap-2 py-3 text-sm font-semibold transition-colors', isFav ? 'bg-red-50 text-red-600' : 'bg-white text-neutral-600 hover:bg-neutral-50')}>
              <Heart className={classNames('h-4 w-4', isFav && 'fill-current')} /> {isFav ? 'Favorited' : 'Add to favorites'}
            </button>
          )}
        </div>
        ...
        <div>
          <div className="flex flex-wrap gap-2">
            {family && <span className="rounded-full bg-brand-purple/10 px-2.5 py-0.5 text-[11px] font-semibold text-brand-purple">{family.name}</span>}
            {category && <span className="rounded-full bg-neutral-100 px-2.5 py-0.5 text-[11px] font-semibold text-neutral-600">{category.name}</span>}
            {color.is_interior && <span className="rounded-full bg-accent-green/10 px-2.5 py-0.5 text-[11px] font-semibold text-accent-green">Interior</span>}
            {color.is_exterior && <span className="rounded-full bg-accent-cyan/10 px-2.5 py-0.5 text-[11px] font-semibold text-accent-cyan">Exterior</span>}
            {color.is_trending && <span className="rounded-full bg-accent-orange/10 px-2.5 py-0.5 text-[11px] font-semibold text-accent-orange">Trending</span>}
      ...
      {related.length > 0 && (
        <div className="mt-12">
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8">
            {related.map((c) => (
      ...
      <ColorRelationships color={color} allColors={allColors} overrides={overrides} />
    </div>
  );
}

function ColorRelationships({ color, allColors, overrides }: { color: DbPaintColor; allColors: DbPaintColor[]; overrides: DbColorRelationshipOverride[] }) {
  const hex = normalizeHex(color.hex_code);
  const overrideMap = new Map(overrides.map((o) => [o.relationship_type, o.override_color_ids]));

  function getColors(type: ColorRelationshipType, computedHexes: string[]): DbPaintColor[] {
    const overrideIds = overrideMap.get(type);
    if (overrideIds && overrideIds.length > 0) {
      return overrideIds.map((id) => allColors.find((c) => c.id === id)).filter(Boolean) as DbPaintColor[];
    }
    return computedHexes.map((h) => allColors.find((c) => normalizeHex(c.hex_code).toUpperCase() === h.toUpperCase())).filter(Boolean) as DbPaintColor[];
  }

  const sections: { type: ColorRelationshipType; title: string; colors: DbPaintColor[] }[] = [
    { type: 'complementary', title: 'Complementary', colors: getColors('complementary', [complementaryColor(hex)]) },
    { type: 'analogous', title: 'Analogous', colors: getColors('analogous', analogousColors(hex)) },
    { type: 'triadic', title: 'Triadic', colors: getColors('triadic', triadicColors(hex)) },
    { type: 'lighter', title: 'Lighter Alternatives', colors: getColors('lighter', [lighterColor(hex, 15), lighterColor(hex, 25)]) },
    { type: 'darker', title: 'Darker Alternatives', colors: getColors('darker', [darkerColor(hex, 15), darkerColor(hex, 25)]) },
    { type: 'matching_trim', title: 'Matching Trim Colors', colors: getColors('matching_trim', [matchingTrimColor(hex)]) },
    { type: 'matching_ceiling', title: 'Matching Ceiling Colors', colors: getColors('matching_ceiling', [matchingCeilingColor(hex)]) },
    { type: 'coordinated_accent', title: 'Coordinated Accent Colors', colors: getColors('coordinated_accent', [coordinatedAccentColor(hex)]) },
  ];
