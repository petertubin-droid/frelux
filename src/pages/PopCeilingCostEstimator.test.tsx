...
vi.mock("@/lib/queries", () => ({ logAnalyticsEvent: vi.fn().mockResolvedValue(null), fetchPopMaterials: vi.fn().mockResolvedValue({ data: [], error: null }), fetchSiteSettings: vi.fn().mockResolvedValue({ data: null, error: null }), saveUserProject: vi.fn().mockResolvedValue({ data: null, error: null }) }));
...
async function renderPage() {
  const Comp = (await import("@/pages/PopCeilingCostEstimator")).default;
  return render(
    <MemoryRouter>
      <ToastProvider>
        <Comp />
...
describe("PopCeilingCostEstimator", () => {
