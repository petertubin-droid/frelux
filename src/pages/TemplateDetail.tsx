    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="prose prose-sm mt-6 max-w-none dark:prose-invert">
        <p>
          ...
          the FRELUX {calcLabel} calculator will open with these inputs pre-filled.
          The calculator then runs the current calculation engine using today's material prices and coverage rules
          specific to Nigerian construction practices.
        </p>
      </div>
      ...
      <div className="prose prose-sm mt-6 max-w-none dark:prose-invert">
        ...
        <p>
          When you click "Use This Template," the {calcLabel} opens with the configuration above already filled in.
          You can adjust any value to match your actual room or surface dimensions. The calculator uses
          Nigerian coverage rates, real product sizes, and current market prices to compute material quantities
          and cost estimates. Every formula, waste factor, and coverage rate is transparent and visible in the results.
        </p>
      </div>
      ...
      {/* FAQ */}
      <div className="mt-10">
        <h2 className="mb-4 font-display text-lg font-bold text-neutral-900 dark:text-white">Frequently Asked Questions</h2>
...
function buildInputSummary(template: DbCalculatorTemplate): { label: string; value: string }[] {
  ...
  if (template.calculator_type === 'paint') {
    ...
    if (d.length) parts.push({ label: 'Length', value: `${d.length} ${d.unit ?? ''}` });
    if (d.width) parts.push({ label: 'Width', value: `${d.width} ${d.unit ?? ''}` });
    if (d.tileWidthMm) parts.push({ label: 'Tile Size', value: `${d.tileWidthMm}x${d.tileHeightMm}mm` });
