      ...
      {loading ? <StateMessage type="loading" title="Loading…" message="Fetching analytics events." />
        ...
        : (
          <>
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <AdminCard>
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">Total events</p>
                </div>
                <p className="mt-1 text-2xl font-bold text-brand-navy dark:text-white">{totalEvents}</p>
                <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-500">Event count, not unique visitors</p>
              </AdminCard>
              <AdminCard>
                <div className="flex items-center gap-2">
                  <BarChart3 aria-hidden="true" className="h-5 w-5 text-accent-green" />
                  <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">AI successes</p>
                </div>
                <p className="mt-1 text-2xl font-bold text-brand-navy dark:text-white">{aiSuccess}</p>
                <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-500">Recommendations generated</p>
              </AdminCard>
              <AdminCard>
                <div className="flex items-center gap-2">
                  <AlertTriangle className="h-5 w-5 text-red-500" />
                  <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">AI failures</p>
                </div>
                <p className="mt-1 text-2xl font-bold text-brand-navy dark:text-white">{aiFailed}</p>
                <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-500">Failed AI requests</p>
              </AdminCard>
              <AdminCard>
                <div className="flex items-center gap-2">
                  <Mail aria-hidden="true" className="h-5 w-5 text-accent-green" />
                  <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">WhatsApp clicks</p>
                </div>
                <p className="mt-1 text-2xl font-bold text-brand-navy dark:text-white">{counts['whatsapp_clicked'] ?? 0}</p>
                <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-500">Contact via WhatsApp</p>
              </AdminCard>
            </div>
                ...
                return (
                  <AdminCard key={cat.label}>
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">{cat.label}</p>
                    </div>
                    ...
                    <div className="mt-2 space-y-1">
                      {cat.events.map((e) => (
                        <div key={e} className="flex items-center justify-between text-xs">
                          <span className="text-neutral-500 dark:text-neutral-500">{e}</span>
              ...
              <div className="mb-6">
                <AdminCard>
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">Usage limit reached</p>
                  </div>
                  <p className="mt-1 text-2xl font-bold text-brand-navy dark:text-white">{aiLimitReached}</p>
                  <p className="mt-0.5 text-xs text-neutral-500 dark:text-neutral-500">Times users hit the daily AI limit</p>
                </AdminCard>
              </div>
            )}
            ...
            {/* Recent events table */}
            <AdminCard>
              <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-500">Recent events</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead><tr className="border-b border-neutral-200 text-xs uppercase tracking-widest text-neutral-500 dark:text-neutral-500"><th className="py-2 pr-4">Event</th><th className="py-2 pr-4">Page</th><th className="py-2 pr-4">Time</th></tr></thead>
                  <tbody>
                    {events.slice(0, 50).map((e) => (
                      <tr key={e.id} className="border-b border-neutral-100">
                        <td className="py-2 pr-4 font-medium text-brand-navy dark:text-white">{e.event}</td>
                        <td className="py-2 pr-4 text-neutral-500 dark:text-neutral-500">{e.page_path ?? 'N/A'}</td>
                        <td className="py-2 pr-4 text-neutral-500 dark:text-neutral-500">{new Date(e.created_at).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <p className="mt-3 text-xs text-neutral-500 dark:text-neutral-500">Showing the 50 most recent events. Total events logged: {totalEvents}.</p>
            </AdminCard>
          </>
        )
      }
