  ...
  return (
    <div>
      ...
      <div className="mb-4 rounded-md bg-amber-50 px-4 py-2 text-sm text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
        ⚠️ Nigeria (NG) is the only active market. Ghana (GH) and Kenya (KE)
        remain inactive until their rules and market data are configured and
        approved.
      </div>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-3 py-2">
                  ...
                  {COVERAGE_TYPE_LABELS[item.coverage_type]}
                </td>
                <td className="px-3 py-2">
    ...
    <div className="mb-4 rounded-lg border border-border bg-card p-4 dark:border-white/10 dark:bg-background">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        <Input
          ...
          onChange={(v) => setForm({ ...form, material_key: v })}
        />
        <Input
          label="Product Name"
        ...
        <Select
          ...
          onChange={(v) =>
            setForm({
              ...form,
              coverage_type: v as
                "count" | "linear" | "area" | "volume" | "weight",
            })
          }
          options={Object.entries(COVERAGE_TYPE_LABELS)}
        />
        <Input
          label="Coverage Value"
          type="number"
          value={String(form.coverage_value)}
          onChange={(v) => setForm({ ...form, coverage_value: Number(v) })}
        />
        <Input
          label="Coverage Unit"
          value={form.coverage_unit}
          onChange={(v) => setForm({ ...form, coverage_unit: v })}
        />
        <Input
          label="Coats"
          type="number"
          value={String(form.coverage_coats)}
          onChange={(v) => setForm({ ...form, coverage_coats: Number(v) })}
        />
        <Input
          label="Package Size"
          type="number"
          value={String(form.package_size)}
          onChange={(v) => setForm({ ...form, package_size: Number(v) })}
        />
        <Input
          label="Package Unit"
        ...
        <Input
          label="Waste %"
          type="number"
          value={String(form.default_waste_percent)}
          onChange={(v) =>
            setForm({ ...form, default_waste_percent: Number(v) })
          }
        />
        <Input
          label="Market"
          value={form.market_code}
          onChange={(v) => setForm({ ...form, market_code: v })}
        />
      </div>
              ...
              {sections.map((s) => (
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    {s.is_flat
                      ? "Flat"
                      : `${s.pitch_value}:${s.pitch_ratio_run} (${s.pitch_type})`}
                  </td>
...
function WasteTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setConfigs(await fetchWasteConfigs());
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  return (
    <div>
      <div className="mb-4 rounded-md bg-blue-50 px-4 py-2 text-sm text-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
        Waste is resolved by hierarchy: Global → Country → Market → Category →
        Rule. More specific scopes override less specific ones.
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
...
function RulesTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await fetchRuleMetadata());
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  async function handleVerify(id: string) {
    try {
      await verifyRuleMetadata(id, user?.id ?? "");
      load();
    } catch (e) {
      console.error(e);
    }
...
function SettingsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await fetchEngineSettings());
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
        ...
        <tbody>
          {items.map((item) => (
            ...
            >
              ...
              <td className="px-3 py-2">
                {SETTING_CATEGORY_LABELS[item.category] || item.category}
              </td>
              <td className="px-3 py-2">
                  ...
                  (editing[item.setting_key] !== undefined ? (
                    ...
                    <button
                      onClick={() =>
                        setEditing({
                          ...editing,
                          [item.setting_key]: JSON.stringify(
                            item.setting_value,
                          ),
                        })
                      }
...
function MarketsTab() {
  const [markets, setMarkets] = useState<
    { country_code: string; country_name: string; status: string }[]
  >([]);
  const [loading, setLoading] = useState(true);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setMarkets(await fetchMarketActivationStatus());
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  async function handleToggle(code: string, currentStatus: string) {
    ...
    if (code !== "NG" && newStatus === "active") {
      if (
        !confirm(
          `Activate ${code}? Ensure all rules and market data are configured and approved first.`,
        )
      )
        return;
    }
    try {
      await toggleMarketActivation(
        code,
        newStatus as "active" | "coming_soon" | "unsupported" | "test_only",
      );
      load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <div>
      <div className="mb-4 rounded-md bg-amber-50 px-4 py-2 text-sm text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
        ⚠️ Activating a market requires fully configured and approved rules,
        material profiles, and pricing data. Do NOT activate international
        markets prematurely.
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
