...
import {
  Sparkles, Send, Loader2, AlertCircle, Check, ExternalLink,
  Settings, MessageSquare, Key, Plus, History, Bug, Zap,
  Wrench, FileText, Code, ShieldCheck, RefreshCw, ChevronDown,
} from 'lucide-react';
...
const STATUS_COLORS: Record<string, string> = {
  ...
  closed: 'bg-neutral-100 text-neutral-500 dark:bg-white/5 dark:text-neutral-400',
  failed: 'bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400',
};

const AGENT_CHAT_URL = 'https://app.base44.com/superagent/6a872e1df3b5e9fc45fc13fb';
...
export default function AdminAIAssistant() {
  ...
  async function handleSend(templatePrompt?: string) {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('ai-admin-assistant', {
        body: {
          message: messageText,
          conversationId,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            Powered by Solas — your FRELUX Superagent. Describe any issue and get it fixed without leaving your admin.
          </p>
        </div>
        <a
          href={AGENT_CHAT_URL}
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-amber-600 dark:text-amber-500">
                    To connect your admin to Solas, you need to add your Superagent API key.
                    Go to the <button onClick={() => setTab('settings')} className="underline font-medium">Settings tab</button> to configure it.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-4 dark:border-white/10 dark:bg-brand-navy-mid">
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              >
                {sending ? (
                  <><Loader2 className="h-4 w-4 animate-spin" /> Solas is working...</>
                ) : (
      ...
      {tab === 'history' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-neutral-900 dark:text-white">Recent Actions</h2>
            <button
              onClick={loadActions}
            ...
            >
              <RefreshCw className={classNames('h-4 w-4', actionsLoading && 'animate-spin')} /> Refresh
            </button>
          </div>
          ...
          {actionsLoading ? (
            <div className="space-y-3">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-20 animate-pulse rounded-lg bg-neutral-100 dark:bg-white/5" />
              ))}
            </div>
          ) : actions.length === 0 ? (
            <p className="rounded-lg border border-neutral-200 py-8 text-center text-sm text-neutral-400 dark:border-white/5 dark:text-neutral-500">
              No actions yet. Start a conversation with Solas to track your fixes here.
            </p>
          ) : (
            <div className="space-y-3">
              {actions.map((action) => (
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-neutral-200 p-6 dark:border-white/10">
            ...
            <p className="mb-4 text-sm text-neutral-500 dark:text-neutral-400">
              Connect your admin to Solas (your FRELUX Superagent on Base44). You need the API key from your
              <a href={`${BASE44_URL}/superagent/6a872e1df3b5e9fc45fc13fb`} target="_blank" rel="noopener noreferrer" className="mx-1 text-brand-purple underline">
                Solas agent page
              </a>
              → Developer/API Docs.
            </p>
            ...
            <p className="mt-2 text-xs text-neutral-400">
              The key is stored securely in your database and only accessible to admins.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-neutral-200 p-6 dark:border-white/10">
            ...
            <p className="mb-4 text-sm text-neutral-500 dark:text-neutral-400">
              Need to access Base44 directly? Use these links to jump into your dashboard, builder, or agent settings.
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-neutral-900 dark:text-white">Base44 Dashboard</p>
                  <p className="text-xs text-neutral-500">Main dashboard & app list</p>
                </div>
              </a>
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-neutral-900 dark:text-white">Solas Chat</p>
                  <p className="text-xs text-neutral-500">Direct chat with Solas</p>
                </div>
              </a>
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-neutral-900 dark:text-white">Agent Settings</p>
                  <p className="text-xs text-neutral-500">Configure Solas, models, memory</p>
                </div>
              </a>
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-neutral-900 dark:text-white">Documentation</p>
                  <p className="text-xs text-neutral-500">Base44 API docs & guides</p>
                </div>
              </a>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-neutral-200 p-6 dark:border-white/10">
            ...
            <p className="mb-4 text-sm text-neutral-500 dark:text-neutral-400">
              Manage external API keys and endpoints used by FRELUX. Changes are saved to your database
              and take effect immediately. Use Solas chat to request API changes — just describe what
              you want to change and Solas will update it for you.
            </p>
            <div className="space-y-3">
              ...
              <div className="rounded-lg border border-neutral-200 p-4 dark:border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-neutral-900 dark:text-white">Supabase Backend</p>
                    <p className="text-xs text-neutral-500">Database, Auth, Edge Functions, Storage</p>
                  </div>
              ...
              <div className="rounded-lg border border-neutral-200 p-4 dark:border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-neutral-900 dark:text-white">Solas Superagent</p>
                    <p className="text-xs text-neutral-500">AI-powered admin assistant & code fixes</p>
                  </div>
                  <span className={classNames(
              ...
              <div className="rounded-lg border border-neutral-200 p-4 dark:border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-neutral-900 dark:text-white">OpenAI (AI Features)</p>
                    <p className="text-xs text-neutral-500">Live chat, color consult, project assistant</p>
                  </div>
