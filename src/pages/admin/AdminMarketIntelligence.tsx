...
import {
  PROVIDER_TYPE_LABELS, SOURCE_TYPE_LABELS, VALIDATION_STATUS_LABELS,
  FRESHNESS_LABELS, ANOMALY_TYPE_LABELS, CONFIDENCE_LABELS,
} from '@/types/market-intelligence';
import { classNames } from '@/lib/utils';
import { useAuth } from '@/lib/auth';

type Tab = 'providers' | 'sources' | 'observations' | 'approved' | 'anomalies' | 'logs';
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-neutral-900 dark:text-white">Market Intelligence</h1>
...
function ObservationsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setObservations(await fetchObservations({ validationStatus: statusFilter === 'all' ? undefined : statusFilter, limit: 200 }));
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {Object.entries(VALIDATION_STATUS_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
          </select>
          <span className="text-sm text-neutral-500">{observations.length} observations</span>
        </div>
        <button
      ...
      {observations.length === 0 ? (
        <p className="py-10 text-center text-sm text-neutral-400">No price observations found.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {observations.map((obs) => (
                <tr key={obs.id} className="border-b border-neutral-100 dark:border-white/5">
                  <td className="py-2 pr-3">
                    <div className="font-medium text-neutral-900 dark:text-white">{obs.original_product_name}</div>
                    {obs.normalized_name && obs.normalized_name !== obs.original_product_name && (
                      <div className="text-xs text-neutral-400">→ {obs.normalized_name}</div>
                    )}
                  </td>
                  <td className="py-2 pr-3 font-semibold">{obs.currency_code} {obs.price.toLocaleString()}</td>
                  <td className="py-2 pr-3 text-xs text-neutral-500">
                    {obs.package_size ? `${obs.package_size} ${obs.package_unit ?? ''}` : '—'}
                  </td>
                  <td className="py-2 pr-3 text-xs">{obs.country_code}{obs.city ? ` / ${obs.city}` : ''}</td>
                  <td className="py-2 pr-3">
                  ...
                  <td className="py-2">
                    {obs.validation_status === 'review_required' || obs.validation_status === 'collected' ? (
                      <div className="flex gap-1">
                        <button
                          onClick={async () => {
                            await updateObservationStatus(obs.id, 'approved', user?.id, 'approved');
                            load();
                          }}
                        ...
                        <button
                          onClick={async () => {
                            await updateObservationStatus(obs.id, 'rejected', user?.id, 'rejected');
                            load();
                          }}
                      ...
                      <span className="text-xs text-neutral-400">{obs.review_action ?? '—'}</span>
                    )}
                  </td>
                </tr>
              ))}
...
function ApprovedTab() {
  ...
  useEffect(() => {
    fetchApprovedPrices().then(setPrices).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-neutral-500">{prices.length} approved prices — these are what calculators consume.</p>
      {prices.length === 0 ? (
        <p className="py-10 text-center text-sm text-neutral-400">No approved prices yet. Approve observations or enter prices manually.</p>
      ) : (
          ...
          {prices.map((p) => (
            <div key={p.id} className="card p-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-bold text-neutral-900 dark:text-white">{p.product_name}</h3>
                  <p className="text-xs text-neutral-400">{p.brand} · {p.category}</p>
                </div>
                <span className={classNames(
                  'rounded-md px-1.5 py-0.5 text-[10px] font-semibold',
                  p.freshness === 'fresh' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' :
                  p.freshness === 'recent' ? 'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400' :
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-lg font-bold text-neutral-900 dark:text-white">{p.currency_code} {p.price.toLocaleString()}</span>
                <span className="text-xs text-neutral-400">{p.package_size} {p.package_unit}</span>
              </div>
              ...
              {p.median_price && p.source_count > 1 && (
                <div className="mt-1 text-xs text-neutral-400">
                  Median: {p.currency_code} {p.median_price.toLocaleString()} · Range: {p.min_price?.toLocaleString()} – {p.max_price?.toLocaleString()}
                </div>
              )}
              <div className="mt-2 flex items-center justify-between border-t border-neutral-100 pt-2 dark:border-white/5">
                <span className="text-xs text-neutral-400">Updated {new Date(p.last_updated).toLocaleDateString()}</span>
                <button
                  onClick={async () => { await deactivateApprovedPrice(p.id); setPrices(prices.filter((x) => x.id !== p.id)); }}
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between">
        <p className="text-sm text-neutral-500">{sources.length} sources registered</p>
        <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-purple px-3 py-1.5 text-sm font-semibold text-white hover:bg-brand-purple-dark">
...
function SourceEditModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({
    source_name: '', domain: '', source_url: '', country_code: 'NG',
    region: '', city: '', source_type: 'supplier' as MiSource['source_type'],
    reliability_tier: 3, crawl_frequency: 'manual' as MiSource['crawl_frequency'],
    is_active: false, is_verified: false, notes: '',
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 dark:bg-brand-navy-mid" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">New Source</h2>
        {error && <div className="mb-3 rounded-lg bg-red-50 p-2 text-sm text-red-600 dark:bg-red-500/10">{error}</div>}
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input label="Region" value={form.region} onChange={(v) => setForm({ ...form, region: v })} placeholder="Lagos" />
          <Input label="City" value={form.city} onChange={(v) => setForm({ ...form, city: v })} />
          <SelectInput label="Reliability Tier" value={String(form.reliability_tier)} onChange={(v) => setForm({ ...form, reliability_tier: parseInt(v) })}
            options={[{ value: '1', label: 'Tier 1 (Manufacturer/Supplier)' }, { value: '2', label: 'Tier 2 (Retailer/Distributor)' }, { value: '3', label: 'Tier 3 (Marketplace)' }, { value: '4', label: 'Tier 4 (Unknown)' }]} />
          <SelectInput label="Crawl Frequency" value={form.crawl_frequency} onChange={(v) => setForm({ ...form, crawl_frequency: v as MiSource['crawl_frequency'] })}
            options={[{ value: 'manual', label: 'Manual' }, { value: 'daily', label: 'Daily' }, { value: 'weekly', label: 'Weekly' }, { value: 'monthly', label: 'Monthly' }]} />
        </div>
        <label className="mt-3 flex items-center gap-2 text-sm text-neutral-700 dark:text-neutral-200">
          <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="h-4 w-4 rounded border-neutral-300 text-brand-purple" />
          Active source
        </label>
...
function ProvidersTab() {
  ...
  useEffect(() => {
    fetchProviders().then(setProviders).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-neutral-500">
        {providers.length} providers configured. Provider API keys are stored in Supabase secrets, never in the database.
      </p>
      <div className="space-y-2">
        {providers.map((p) => (
          <div key={p.id} className="card flex items-center justify-between p-4">
            <div className="flex items-center gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-neutral-900 dark:text-white">{p.provider_name}</span>
                  <span className="rounded-md bg-brand-purple/10 px-1.5 py-0.5 text-[10px] font-semibold text-brand-purple">
                    {PROVIDER_TYPE_LABELS[p.provider_type]}
                  </span>
                  {p.is_free && <span className="text-[10px] font-semibold text-emerald-600">FREE</span>}
                  {p.is_fallback && <span className="text-[10px] text-neutral-400">fallback</span>}
                </div>
                <p className="text-xs text-neutral-400">{p.description}</p>
                <div className="mt-1 flex items-center gap-3 text-xs text-neutral-400">
                  <span>Priority: {p.priority}</span>
                  {p.has_api_key && <span className="text-amber-500">⚠ API key configured</span>}
                  {p.monthly_request_limit && <span>Limit: {p.monthly_request_limit}/mo</span>}
                </div>
              </div>
            </div>
            <button
              onClick={async () => { await toggleProvider(p.id, !p.is_enabled); setProviders(providers.map((x) => x.id === p.id ? { ...x, is_enabled: !x.is_enabled } : x)); }}
              className={classNames(
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                p.is_enabled ? 'bg-brand-purple' : 'bg-neutral-300 dark:bg-white/10'
              )}
            >
              <span className={classNames('inline-block h-4 w-4 transform rounded-full bg-white transition-transform', p.is_enabled ? 'translate-x-6' : 'translate-x-1')} />
            </button>
          </div>
        ))}
      </div>
...
function AnomaliesTab() {
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-neutral-500">{anomalies.length} anomaly flags</p>
      {anomalies.length === 0 ? (
        <p className="py-10 text-center text-sm text-neutral-400">No anomalies detected.</p>
      ) : (
        <div className="space-y-2">
          {anomalies.map((a) => (
            <div key={a.id} className="card p-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {a.deviation_percent && (
                    <span className="text-xs text-amber-600">{a.deviation_percent}% deviation</span>
                  )}
                </div>
                <span className={classNames(
                  'rounded-md px-2 py-0.5 text-[10px] font-semibold',
                  a.resolution === 'open' ? 'bg-amber-50 text-amber-600 dark:bg-amber-500/10' :
                  a.resolution === 'resolved' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10' :
              ...
              {a.resolution === 'open' && (
                <div className="mt-2 flex gap-2">
                  <button
                    onClick={async () => { await resolveAnomaly(a.id, 'resolved', user?.id ?? ''); load(); }}
                    className="rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-600 dark:bg-emerald-500/10"
                  >
                    Resolve
                  </button>
                  <button
                    onClick={async () => { await resolveAnomaly(a.id, 'dismissed', user?.id ?? ''); load(); }}
...
function LogsTab() {
  ...
  useEffect(() => {
    fetchCrawlLogs(200).then(setLogs).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-neutral-500">Recent system activity ({logs.length} events)</p>
      <div className="space-y-1">
        {logs.slice(0, 50).map((log) => (
          <div key={log.id} className="flex items-start gap-2 border-b border-neutral-100 py-2 dark:border-white/5">
            <div className="text-xs text-neutral-400" style={{ minWidth: '120px' }}>
              {new Date(log.created_at).toLocaleString()}
            </div>
            <span className="rounded-md bg-brand-purple/10 px-1.5 py-0.5 text-[10px] font-semibold text-brand-purple">
              {log.event_type}
            </span>
            <span className="text-xs text-neutral-600 dark:text-neutral-400">{log.message}</span>
          </div>
        ))}
      </div>
    </div>
...
function ManualEntryModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const { user } = useAuth();
  const [form, setForm] = useState({
    market_code: 'NG', country_code: 'NG', source_name: 'Manual Admin Entry',
    original_name: '', normalized_name: '', brand: '', category: '',
    package_size: '', package_unit: 'kg', price: '', currency: 'NGN',
    region: '', city: '',
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
    ...
    try {
      ...
      await manuallyEnterPrice(
        ...
        manualSource.id,
        {
          ...
          packageSize: form.package_size ? parseFloat(form.package_size) : undefined,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 dark:bg-brand-navy-mid" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Manual Price Entry</h2>
        {error && <div className="mb-3 rounded-lg bg-red-50 p-2 text-sm text-red-600 dark:bg-red-500/10">{error}</div>}
        <div className="grid grid-cols-2 gap-3">
          <Input label="Market Code" value={form.market_code} onChange={(v) => setForm({ ...form, market_code: v.toUpperCase(), country_code: v.toUpperCase() })} />
          <Input label="Currency" value={form.currency} onChange={(v) => setForm({ ...form, currency: v.toUpperCase() })} />
          <Input label="Product Name (raw)" value={form.original_name} onChange={(v) => setForm({ ...form, original_name: v })} placeholder="Portland Cement 50kg" />
          <Input label="Normalized Name" value={form.normalized_name} onChange={(v) => setForm({ ...form, normalized_name: v })} placeholder="Portland Cement" />
          <Input label="Brand" value={form.brand} onChange={(v) => setForm({ ...form, brand: v })} placeholder="Dangote" />
          <Input label="Category" value={form.category} onChange={(v) => setForm({ ...form, category: v })} placeholder="cement" />
          <Input label="Package Size" value={form.package_size} onChange={(v) => setForm({ ...form, package_size: v })} placeholder="50" />
          <Input label="Package Unit" value={form.package_unit} onChange={(v) => setForm({ ...form, package_unit: v })} placeholder="kg" />
          <Input label="Price" value={form.price} onChange={(v) => setForm({ ...form, price: v })} placeholder="9800" />
          <Input label="Region" value={form.region} onChange={(v) => setForm({ ...form, region: v })} placeholder="Lagos" />
        </div>
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-lg border border-neutral-200 px-4 py-2 text-sm font-semibold text-neutral-600 dark:border-white/10 dark:text-neutral-400">Cancel</button>
          <button onClick={handleSave} disabled={saving || !form.normalized_name || !form.price} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-purple px-4 py-2 text-sm font-semibold text-white hover:bg-brand-purple-dark disabled:opacity-50">
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save Price
          </button>
        </div>
      </div>
    </div>
...
function ConfidenceBadge({ confidence }: { confidence: MatchConfidence }) {
  const colors: Record<MatchConfidence, string> = {
    ...
    review_required: 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400',
  };
  return (
    <span className={classNames('rounded-md px-1.5 py-0.5 text-[10px] font-semibold', colors[confidence])}>
...
function StatusBadge({ status }: { status: ValidationStatus }) {
  const colors: Record<ValidationStatus, string> = {
    collected: 'bg-neutral-100 text-neutral-500 dark:bg-white/5 dark:text-neutral-400',
    validating: 'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400',
    review_required: 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400',
    approved: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400',
    rejected: 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400',
    anomaly: 'bg-orange-50 text-orange-600 dark:bg-orange-500/10 dark:text-orange-400',
  };
  return (
    <span className={classNames('rounded-md px-1.5 py-0.5 text-[10px] font-semibold', colors[status])}>
