...
const ACCESS_MODES: {
...
}[] = [
  {
    value: "free",
    label: "Free",
    description: "Anyone can use AI features within the daily limit.",
  },
  {
    value: "rewarded",
    label: "Rewarded Access",
    description:
      "After the daily free limit, users can unlock more via rewarded access.",
  },
  {
    value: "paid",
    label: "Paid",
    description: "AI features require payment (future integration).",
  },
  {
    value: "free_rewarded",
    label: "Free + Rewarded",
    description: "Free daily uses, then rewarded access option.",
  },
  {
    value: "disabled",
    label: "Disabled",
    description: "AI features are turned off for everyone.",
  },
];
...
export default function AdminAiMonetization() {
  ...
  const [paymentProviderConfigured, setPaymentProviderConfigured] =
    useState(false);
  ...
  async function onSave() {
    ...
    if (settings.ai_access_mode === "paid" && !paymentProviderConfigured) {
      setError(
        "Cannot enable Paid mode, no payment provider is configured. Connect a payment provider (Paystack, Flutterwave, or Stripe) first.",
      );
      return;
    }
    setSaving(true);
    ...
    return (
      <>
        <AdminHeader
          ...
          subtitle="Configure AI feature access, daily limits, and monetization options."
        />
        <StateMessage
          type="loading"
          title="Loading…"
  ...
  const showPaidWarning =
    settings.ai_access_mode === "paid" && !paymentProviderConfigured;
  ...
  return (
    <>
      <AdminHeader
        ...
        subtitle="Configure AI feature access, daily limits, and monetization options."
        action={
          <AdminButton onClick={onSave} disabled={saving || !canSave()}>
      ...
      {savedAt && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-700">
          <CheckCircle2 aria-hidden="true" className="h-4 w-4" /> AI settings
          saved.
        </div>
      )}
      {error && (
        ...
        <div className="mb-4 flex items-start gap-3 rounded-lg border border-orange-200 bg-orange-50 p-4 text-sm text-orange-700">
          ...
          <div>
            ...
            <p className="mt-1 text-xs">
              Paid mode is selected but no payment provider (Paystack,
              Flutterwave, Stripe) is configured. Users will be completely
              locked out of AI features with no way to pay. Switch to Free or
              Rewarded mode, or connect a payment provider first.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground dark:text-muted-foreground">
                    Master switch for all Smart Color Assistant features.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {ACCESS_MODES.map((m) => (
              <AdminButton
                ...
                className={
                  "flex flex-col items-start gap-1 rounded-lg border p-4 text-left font-normal transition-all " +
                  (settings.ai_access_mode === m.value
        ...
        <AdminCard>
          ...
          <p className="mb-4 text-xs text-muted-foreground dark:text-muted-foreground">
            This limit is shared across all AI features (text consultation,
            image analysis). Only successful generations consume a use.
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            <AdminField
              label="Daily free AI uses"
              hint="Default: 3. Shared across all AI features."
            >
              <AdminInput
                type="number"
                min={0}
                ...
                onChange={(e) =>
                  update("ai_daily_free_uses", Number(e.target.value))
                }
              />
            </AdminField>
        ...
        <AdminCard>
          <div className="flex items-center gap-2">
            <Gift aria-hidden="true" className="h-4 w-4 text-accent-orange" />
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground dark:text-muted-foreground">
              Rewarded Access
            </h2>
          </div>
          <p className="mt-2 text-xs text-muted-foreground dark:text-muted-foreground">
            Provider agnostic architecture. No rewarded provider is connected
            yet, the UI shows an unavailable state. Never grants access on a
            button click alone.
          </p>
          <div className="mt-3 flex items-center gap-3">
            <Toggle
            ...
            <span className="text-sm text-muted-foreground">
              Rewarded access available (shows option to users)
            </span>
          </div>
        </AdminCard>
        ...
        <AdminCard>
            ...
            <Toggle
              ...
              onChange={(v) => {
                ...
                if (v && !paymentProviderConfigured) {
                  setError(
                    "Cannot enable Paid access, no payment provider is configured. Connect a payment provider first.",
                  );
                  return;
                }
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <AdminField
              ...
              hint={
                paymentProviderConfigured
                  ? "Price per AI access period."
                  : "Placeholder, not active until a payment provider is connected."
              }
            >
              <AdminInput
                type="number"
                ...
                onChange={(e) =>
                  update("ai_paid_price", Number(e.target.value))
                }
        ...
        <AdminCard>
          ...
          <p className="mt-2 text-xs text-muted-foreground dark:text-muted-foreground">
            When enabled, admin users bypass AI usage limits for testing.
          </p>
          <div className="mt-3 flex items-center gap-3">
            <Toggle
