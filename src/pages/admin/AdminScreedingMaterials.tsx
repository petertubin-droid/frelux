...
export default function AdminScreedingMaterials() {
  ...
  async function onSave() {
    ...
    const { error: updateError } = await supabase
      .from('screeding_mix_config')
      .update({
        paint_coverage_rate_m2_per_l: Math.max(0.1, Number(paintCoverage) || 0.1),
        paint_bucket_size_l: Math.max(1, Number(paintBucketSize) || 1),
        paint_price_per_bucket: Math.max(0, Number(paintPrice) || 0),
        cement_consumption_ratio_kg_per_l: Math.max(0, Number(cementRatio) || 0),
        cement_bag_size_kg: Math.max(1, Number(cementBagSize) || 1),
        cement_price_per_bag: Math.max(0, Number(cementPrice) || 0),
        default_mix_ratio: mixRatio,
        labour_rate_per_sqm: Math.max(0, Number(labourRate) || 0),
        waste_percentage: Math.max(0, Math.min(100, Number(wastePct) || 0)),
        tax_vat_percentage: Math.max(0, Math.min(100, Number(taxPct) || 0)),
        currency,
