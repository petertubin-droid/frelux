...
export default function AdminScreedingMaterials() {
  ...
  const [puttyConfig, setPuttyConfig] =
    useState<DbScreedingSystemConfig | null>(null);
  const [mixConfig, setMixConfig] = useState<DbScreedingSystemConfig | null>(
    null,
  );
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setError(getSafeError(err, "Failed to load screeding configuration."));
      }
      setLoading(false);
    }
    load();
  ...
  async function saveConfig(
    config: DbScreedingSystemConfig,
  ...
  if (loading)
    return (
      <StateMessage
        type="loading"
        title="Loading…"
        message="Fetching screeding material system configurations."
      />
    );
  ...
  return (
    <>
      ...
      <div className="mb-6 flex gap-2 border-b border-border dark:border-white/10">
        <TabButton
          active={activeTab === "putty"}
          onClick={() => setActiveTab("putty")}
          icon={<PaintBucket aria-hidden="true" className="h-4 w-4" />}
          label="Putty"
        />
        <TabButton
          active={activeTab === "white_cement_paint"}
          onClick={() => setActiveTab("white_cement_paint")}
          icon={<Paintbrush aria-hidden="true" className="h-4 w-4" />}
          label="White Cement + Paint"
        />
      </div>
      ...
      {activeTab === "putty" && puttyConfig && (
        <PuttyConfigPanel
          config={puttyConfig}
          onSave={(updates) => saveConfig(puttyConfig, updates)}
          saving={saving}
        />
      )}
      {activeTab === "putty" && !puttyConfig && (
        <StateMessage
          ...
          message="No active Putty configuration was found in the database. Please run the latest migration."
        />
      )}
      ...
      {activeTab === "white_cement_paint" && mixConfig && (
        <MixConfigPanel
          config={mixConfig}
          onSave={(updates) => saveConfig(mixConfig, updates)}
          saving={saving}
        />
      )}
      {activeTab === "white_cement_paint" && !mixConfig && (
        <StateMessage
          type="error"
          title="No configuration found"
          message="No active White Cement + Screeding Paint configuration was found in the database. Please run the latest migration."
        />
      )}
    </>
  );
...
function PuttyConfigPanel({
  ...
  const [puttyQuantity, setPuttyQuantity] = useState(
    Number(config.putty_quantity ?? 2),
  );
  const [puttyUnit, setPuttyUnit] = useState(config.putty_unit ?? "bucket");
  const [puttyPrice, setPuttyPrice] = useState(
    Number(config.putty_price_per_unit ?? 0),
  );
    ...
    <>
      <AdminCard className="mb-4 p-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-foreground dark:text-primary-foreground">
            Putty Material Configuration
          </h2>
        </div>
        <p className="mt-1 text-xs text-muted-foreground dark:text-muted-foreground">
          Configure the Putty screeding system. The calculator uses these values
          to determine how much Putty is needed based on wall surface area.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <AdminField
            label="Display name"
            hint="Shown to users in the material selector"
          >
            <AdminInput
              type="text"
              value={displayName}
      ...
      <AdminCard className="mb-4 p-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-foreground dark:text-primary-foreground">
            Coverage & Coating Rules
          </h2>
        </div>
        <p className="mt-1 text-xs text-muted-foreground dark:text-muted-foreground">
          The coverage rule defines how much area one unit group covers. Coats
          multiply the base requirement.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <AdminField
            label="Coverage area (m²)"
            hint="Area covered per unit group"
          >
            <AdminInput
              type="number"
              min={0.01}
              step="0.5"
              value={coverageArea}
              onChange={(e) => setCoverageArea(Number(e.target.value))}
            />
          </AdminField>
          ...
          <AdminField label="Default coats" hint="Default number of coats">
            <AdminInput
              ...
              max={10}
              value={defaultCoats}
              onChange={(e) => setDefaultCoats(Number(e.target.value))}
            />
          </AdminField>
          <AdminField
            label="Waste percentage (%)"
            hint="Extra material allowance for waste"
          >
            <AdminInput
              type="number"
              min={0}
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <AdminField label="Material name">
            <AdminInput
              type="text"
              value={puttyName}
              onChange={(e) => setPuttyName(e.target.value)}
            />
          </AdminField>
          <AdminField
            label="Required quantity per coverage area"
            hint="Buckets needed per coverage area"
          >
            <AdminInput
              type="number"
              min={0}
              step="0.5"
              value={puttyQuantity}
              onChange={(e) => setPuttyQuantity(Number(e.target.value))}
            />
          </AdminField>
          <AdminField label="Unit" hint="e.g. bucket, bag, kg">
            <AdminInput
              type="text"
              value={puttyUnit}
              onChange={(e) => setPuttyUnit(e.target.value)}
            />
          </AdminField>
          <AdminField label="Price per unit">
            <AdminInput
              ...
              min={0}
              value={puttyPrice}
              onChange={(e) => setPuttyPrice(Number(e.target.value))}
            />
          </AdminField>
        </div>
      </AdminCard>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-4">
          ...
          <AdminField label="Rounding rule">
            <select
              ...
              value={roundingRule}
              onChange={(e) =>
                setRoundingRule(e.target.value as "ceil" | "none")
              }
            >
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">
          Current price: {formatCurrency(puttyPrice, currencySymbol)} per{" "}
          {puttyUnit}
        </span>
      </div>
    </>
    ...
    <>
      <AdminCard className="mb-4 p-5">
        <div className="flex items-center gap-2">
          ...
          />
          <h2 className="text-sm font-bold text-foreground dark:text-primary-foreground">
            White Cement + Screeding Paint Configuration
          </h2>
        </div>
        <p className="mt-1 text-xs text-muted-foreground dark:text-muted-foreground">
          Configure the combined White Cement and Screeding Paint system. Both
          materials are calculated from the same coverage rule.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <AdminField
            label="Display name"
      ...
      <AdminCard className="mb-4 p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground dark:text-muted-foreground">
          For every coverage area, the configured quantities of Screeding Paint
          and White Cement are needed. Coats multiply the base requirement.
        </p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <AdminField
            label="Coverage area (m²)"
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          >
            <AdminInput
              ...
              step="0.5"
              value={paintQuantity}
              onChange={(e) => setPaintQuantity(Number(e.target.value))}
            />
          </AdminField>
          <AdminField label="Unit" hint="e.g. bucket">
            <AdminInput
              type="text"
              value={paintUnit}
              onChange={(e) => setPaintUnit(e.target.value)}
            />
          </AdminField>
          <AdminField label="Price per unit">
            <AdminInput
      ...
      <AdminCard className="mb-4 p-5">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <AdminField
            label="Quantity per coverage area"
            hint="Bags needed per coverage area"
          >
            <AdminInput
              type="number"
              min={0}
              ...
              onChange={(e) => setCementQuantity(Number(e.target.value))}
            />
          </AdminField>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">
          Paint: {formatCurrency(paintPrice, currencySymbol)}/{paintUnit} ·
          Cement: {formatCurrency(cementPrice, currencySymbol)}/{cementUnit}
        </span>
      </div>
    </>
  );
