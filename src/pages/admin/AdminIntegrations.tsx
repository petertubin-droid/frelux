...
const FIELD_HELP: Record<string, Record<string, string>> = {
  google_analytics: {
    measurement_id:
      "Your GA4 Measurement ID (e.g. G-XXXXXXXXXX). Find it in GA4 → Admin → Data Streams.",
  },
  google_adsense: {
    publisher_id:
  ...
  google_search_console: {
    verification_token:
      "The verification token from Google Search Console. Get it at search.google.com/search-console → Add property → HTML tag method. Paste only the token content, not the full meta tag.",
  },
  media_net: {
  ...
  buysellads: {
    site_key:
      "Your BuySellAds site key. Register at buysellads.com and add your site.",
  ...
  propellerads: {
    zone_id:
      "Your PropellerAds zone ID. Create a zone in the PropellerAds dashboard.",
  ...
  meta_pixel: {
    pixel_id:
      "Your Meta/Facebook Pixel ID (e.g. 123456789012345). Find it in Meta Events Manager → Data Sources → Pixel.",
  },
  revu: {
...
export default function AdminIntegrations() {
  ...
  async function handleToggle(integration: DbIntegrationSetting) {
    ...
    try {
      await updateIntegrationSetting(integration.integration_key, {
  ...
  async function handleSaveConfig(integration: DbIntegrationSetting) {
    ...
    setSavedKeys((prev) => {
      ...
      n.delete(integration.integration_key);
      return n;
    });
    try {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Configure and manage third-party integrations. Toggle each integration
          on, enter your credentials, and save. Changes take effect on the next
          page load.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {["google_analytics", "google_adsense", "google_search_console"].map(
          (key) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <Icon
                    className={`h-4 w-4 ${configured ? "text-emerald-600" : "text-muted-foreground"}`}
                  />
                  <span className="text-sm font-medium">
                    {(integ as DbIntegrationSetting).display_name}
                  </span>
                </div>
          ...
          <div key={category}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {categoryIntegrations.map((integration) => {
                ...
                const configFields = Object.entries(integration.config).filter(
                  ([k]) => k !== "id",
                );
                  ...
                  <Card key={integration.id} className="p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span
                          className={`mt-1 inline-flex items-center gap-1 text-xs ${integration.is_enabled ? "text-emerald-600" : "text-muted-foreground"}`}
                        >
                      ...
                      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        {configFields.map(([key, _value]) => (
                          <div key={key}>
                            ...
                            <Input
                              ...
                              value={
                                editValues[integration.integration_key]?.[
                                  key
                                ] ?? ""
                              }
                              onChange={(e) =>
                        ...
                        <div className="flex items-center gap-3">
                          <Button
                            ...
                            size="sm"
                          >
                            {saving === integration.integration_key ? (
      ...
      <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
        ...
        <ol className="mt-2 space-y-1 text-xs text-blue-800 dark:text-blue-300/80">
          <li>
            ...
            and add your property (${SITE_URL.replace("https://", "")})
          </li>
          <li>2. Choose the "HTML tag" verification method</li>
          <li>
            3. Copy the token value from the meta tag (just the content
            attribute value)
          </li>
          <li>
            4. Enable "Google Search Console" above, paste the token in the
