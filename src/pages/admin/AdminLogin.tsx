...
export default function AdminLogin() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { error: signInError, isAdmin: adminOk } = await signIn(
      email.trim(),
      password,
    );
    setLoading(false);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card p-6 sm:p-8 dark:border-white/5 dark:bg-card">
          ...
          <p className="mt-1 text-sm text-muted-foreground dark:text-muted-foreground">
            {mode === "signin"
              ? "Sign in with an authorized admin account to manage the platform."
              : "Create an account, then ask an existing admin to grant you admin access."}
          </p>
          {!configured && (
            <div className="mt-4 flex items-start gap-2 rounded-lg border border-accent-yellow/30 bg-accent-yellow/10 p-3 text-xs text-card-foreground dark:text-muted-foreground/60">
              ...
              <p>
                ...
                VITE_SUPABASE_ANON_KEY to enable admin access.
              </p>
            </div>
          )}
          {signedUpEmail ? (
            <div className="mt-6 space-y-4">
              <div className="rounded-lg border border-accent-green/30 bg-accent-green/10 p-4 text-sm text-card-foreground dark:text-muted-foreground/60">
                ...
                <p className="mt-1">
                  Your account{" "}
                  <span className="font-semibold">{signedUpEmail}</span> has
                  been registered. New accounts start as a regular user. To get
                  admin access, an existing admin needs to update your role to{" "}
                  <span className="font-semibold">admin</span> in the profiles
                  table.
                </p>
              </div>
              <AdminButton
