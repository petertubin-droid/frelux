...
export default function SharedProject() {
  ...
  useSeo({
    ...
    description:
      "A shared FRELUX project, colors, calculations, and estimates.",
    noIndex: true,
  });
    ...
    (async () => {
      ...
      if (linkData.resource_type === "project") {
        ...
        if (proj) {
          ...
          const savedColors = (proj as Record<string, unknown>).saved_colors as
            { color_id?: string }[] | undefined;
          if (savedColors && savedColors.length > 0) {
            const colorIds = savedColors
              .map((c) => c.color_id)
        ...
        {project && (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold capitalize text-brand-purple">
                {project.project_type.replace("_", " ")}
              </span>
              <span className="flex items-center gap-1 text-xs text-muted-foreground">
                <Clock aria-hidden="true" className="h-3 w-3" /> Created{" "}
                ...
                <div className="mt-3 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                  {colors.map((c) => (
                    <Link
                      ...
                      className="group overflow-hidden rounded-lg border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-md"
                    >
                      <div
                      ...
                      >
                        <span
                          className="flex h-full items-center justify-center text-xs font-bold uppercase"
                  ...
                  {Object.entries(projectData)
                    ...
                    .map(([key, value]) => (
                      <div key={key}>
                        ...
                        <dd className="mt-0.5 text-sm text-card-foreground dark:text-muted-foreground/60">
