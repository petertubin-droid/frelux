...
export default function Colors() {
  useSeo({
    ...
    description:
      "Browse hundreds of professional paint colors and curated color palettes. Filter by color family, room type, style, and more.",
  ...
  useEffect(() => {
    trackColorView();
    trackRecentTool("/colors", "Browse Colors", "Palette");
  });
  ...
  const [sort, setSort] = useState<
    "popularity" | "name" | "newest" | "display_order"
  >("display_order");
  ...
  async function handleToggleFav(colorId: string) {
    ...
    setFavIds((prev) =>
      favorited ? [...prev, colorId] : prev.filter((id) => id !== colorId),
    );
  }
  ...
  const filteredPalettes = palettes.filter((p) => {
    ...
    return (
      ...
      p.description.toLowerCase().includes(paletteQuery.toLowerCase())
    );
  });
          ...
          <>
            ...
            <div className="mt-4 space-y-3">
              ...
              <FilterRow label="Quick filters">
                ...
                <Chip
                  active={filterType === "exterior"}
                  onClick={() => {
                    setFilterType("exterior");
                    setPage(1);
                  }}
                >
                  Exterior
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {colors.map((c) => (
                  <ColorCard
                    key={c.id}
                    color={c}
                    isFavorited={favIds.includes(c.id)}
                    onToggleFavorite={user ? handleToggleFav : undefined}
                  />
                ))}
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        {c.is_trending && (
                          <span className="rounded-full bg-accent-orange/15 px-2.5 py-1 text-[11px] font-semibold text-accent-orange">
                            Trending
                          </span>
                        )}
                        {c.is_featured && (
                          <span className="rounded-full bg-accent-green/10 px-2.5 py-1 text-[11px] font-semibold text-accent-green">
                            Featured
                          </span>
                        )}
                      </div>
