...
function PluginManager() {
  ...
  async function handleToggle(plugin: DbStudioPlugin) {
    ...
    setPlugins((prev) =>
      prev.map((p) =>
        p.id === plugin.id
          ? { ...p, status: newStatus as DbStudioPlugin["status"] }
          : p,
      ),
    );
  }
  ...
  return (
    <div>
      <ToolHeader
        icon={Puzzle}
        title="Plugin & Module Manager"
        description="Install, enable, and manage platform plugins and modules."
      />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {plugins.map((p) => (
          <AdminCard key={p.id} className="flex flex-col">
            ...
            <h3 className="mt-3 text-sm font-bold text-foreground dark:text-primary-foreground">
              {p.name}
            </h3>
            <p className="mt-1 flex-1 text-xs text-muted-foreground dark:text-muted-foreground">
              {p.description}
            </p>
  ...
  return (
    <div>
      <ToolHeader
        ...
        description="Reusable prompt templates for all AI Studio tools."
      />
      {error && (
      ...
      {showForm && (
        <AdminCard className="mb-4">
          <div className="space-y-4">
            ...
            <AdminField label="User Prompt Template">
              <textarea
                className="input-field"
                rows={3}
          ...
          <div className="space-y-2">
            {prompts
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className="group flex items-center justify-between rounded-lg border border-border bg-card dark:border-white/5 dark:bg-card p-4"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {p.is_builtin && (
                        <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground dark:text-muted-foreground">
                          Built in
                        </span>
                      )}
                    </div>
...
function IntegrationCenter() {
  ...
  async function handleToggle(integ: DbStudioIntegration) {
    const newStatus =
      integ.status === "connected" ? "disconnected" : "connected";
    await updateIntegration(integ.id, {
      status: newStatus,
      health_status: newStatus === "connected" ? "healthy" : "unknown",
    });
    ...
    <div>
      ...
      {showForm && (
        <AdminCard className="mb-4">
          ...
          <div className="mt-4">
            <AdminButton onClick={handleCreate} disabled={!formData.name}>
              Add
            </AdminButton>
          </div>
        </AdminCard>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground dark:text-muted-foreground">
                    {i.service_type} · {i.health_status}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
...
function FeatureManagement() {
  ...
  async function handleToggle(f: DbStudioFeature) {
    ...
    setFeatures((prev) =>
      prev.map((x) =>
        x.id === f.id ? { ...x, is_enabled: !x.is_enabled } : x,
      ),
    );
  }
  ...
  async function handleRollout(f: DbStudioFeature, pct: number) {
    ...
    setFeatures((prev) =>
      prev.map((x) => (x.id === f.id ? { ...x, rollout_percentage: pct } : x)),
    );
  }
  ...
  return (
    <div>
      <ToolHeader
        ...
        description="Control feature flags and rollout percentages."
      />
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {error}
          ...
          <AdminCard key={f.id} className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <code className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground dark:text-muted-foreground">
                  {f.feature_key}
                </code>
                <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground dark:text-muted-foreground">
                  {f.category}
                </span>
              </div>
              {f.description && (
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => handleRollout(f, Number(e.target.value))}
                  className="h-1 w-32 cursor-pointer accent-brand-purple"
                />
...
function RoleManagement() {
  ...
  if (loading)
    return (
      <StateMessage type="loading" title="Loading…" message="Fetching roles." />
    );
      ...
      {showForm && (
        <AdminCard className="mb-4">
          <div className="space-y-4">
            ...
            <AdminField
              label="Permissions"
              hint="Comma separated, e.g. studio:*, artifacts:read"
            >
              <input
                className="input-field"
            ...
            <AdminButton onClick={handleCreate} disabled={!formData.role_name}>
              Create Role
            </AdminButton>
          </div>
        </AdminCard>
  ...
  return (
    <div>
      <ToolHeader
        ...
        description="Monitor system health, performance, and usage metrics."
      />
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {error}
      ...
      {metrics.length === 0 ? (
        <StateMessage
          ...
          message="Metrics will appear here as the system generates them."
        />
      ) : (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {metrics
                  ...
                  .map((m) => (
                    <div
                      key={m.id}
                      className="rounded-lg border border-border bg-card dark:border-white/5 dark:bg-card p-4"
                    >
                      <p className="text-xs text-muted-foreground dark:text-muted-foreground">
                        {m.metric_name}
                      </p>
  ...
  return (
    <div>
      <ToolHeader
        ...
        description="View and restore artifact versions over time."
      />
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {error}
          ...
          {artifacts.map((a) => (
            ...
            >
              ...
              <Button
                ...
                className="ml-2 text-muted-foreground/80 hover:text-red-500"
              >
        ...
        <div>
          {selected && versions.length > 0 ? (
            <div className="space-y-3">
              {versions.map((v) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          if (
                            !confirm(
                              `Restore artifact to version ${v.version_number}?`,
                            )
                          )
                            return;
                          try {
                            await updateArtifact(selected, {
                              content: v.content,
                            });
                            setArtifacts((prev) =>
                              prev.map((a) =>
                  ...
                  <pre className="mt-3 max-h-48 overflow-auto rounded-lg bg-muted/50 p-3 text-xs text-card-foreground dark:text-muted-foreground/60">
                    <code>
            ...
            <StateMessage
              ...
              message="Choose an artifact to view its version history."
            />
          )}
        </div>
      </div>
...
function ProjectExplorer() {
  const [tree, setTree] = useState<
    { name: string; path: string; type: string; children?: unknown[] }[]
  >([]);
  ...
  return (
    <div>
      <ToolHeader
        ...
        description="Browse the project structure and file organization."
      />
...
function FileManager() {
  ...
  if (loading)
    return (
      <StateMessage type="loading" title="Loading…" message="Fetching files." />
    );
  ...
  return (
    <div>
      <ToolHeader
        icon={FolderOpen}
        title="File Manager"
        description="Manage generated artifacts and project files."
      />
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {error}
      ...
      {filtered.length === 0 ? (
        <StateMessage
          ...
          message="Generate artifacts using the AI tools to see them here."
        />
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((a) => (
            <div
              key={a.id}
              className="group rounded-lg border border-border bg-card dark:border-white/5 dark:bg-card p-4"
            >
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-foreground dark:text-primary-foreground">
                ...
                <Button
                  ...
                  className="rounded-md p-1.5 text-muted-foreground/80 opacity-0 hover:text-red-500 group-hover:opacity-100"
                >
                  <Trash2 aria-hidden="true" className="h-3.5 w-3.5" />
                </Button>
              </div>
