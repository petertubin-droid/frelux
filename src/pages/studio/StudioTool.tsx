...
import {
  ...
  SaveBar,
  ChatMessage,
} from "@/components/studio/StudioShared";
import type {
  DbStudioChat,
  DbStudioSession,
  StudioToolType,
...
const PLACEHOLDERS: Record<string, string> = {
  page_builder:
    'Describe the page you want to build. e.g., "Create a customer feedback page with a rating form, recent reviews, and analytics summary"',
  crud_generator:
    'Describe the CRUD module. e.g., "Create a CRUD for paint suppliers with name, email, phone, and active status"',
  db_designer:
    'Describe the schema. e.g., "Design tables for a project management system with projects, tasks, and assignees"',
  api_builder:
    'Describe the API. e.g., "Create an endpoint that accepts a webhook from Stripe and logs the event"',
  dashboard_builder:
    'Describe the dashboard. e.g., "Create a sales analytics dashboard with revenue chart, top products, and recent orders"',
  form_builder:
    'Describe the form. e.g., "Create a multi-step contact form with name, email, project type, and message"',
  workflow_builder:
    'Describe the workflow. e.g., "Create an automation that sends an email when a new color is approved"',
  feature_generator:
    'Describe the feature. e.g., "Design a color matching feature that suggests complementary colors"',
  component_generator:
    'Describe the component. e.g., "Create a reusable color picker with hex input and preview swatch"',
  code_generator:
    'Describe what to generate. e.g., "Create a utility function to convert RGB to HSL"',
  deploy_assistant:
    'Describe your deployment needs. e.g., "Help me optimize my Vite build and set up environment variables"',
  bug_detection:
    "Paste the code you want analyzed for bugs and security issues",
  refactoring: "Paste the code you want refactored for better readability",
  test_generator:
    'Describe what to test. e.g., "Generate tests for the paint calculator utility functions"',
  docs_generator:
    'Describe what to document. e.g., "Generate API documentation for the color consultation endpoint"',
};
...
export default function StudioDispatcher() {
  ...
  return (
    <div className="py-20 text-center text-sm text-muted-foreground">
      This tool is available in the sidebar.
    </div>
  );
}
...
function GenerationTool({
  ...
  async function handleGenerate(p: string) {
    ...
    setOutput(null);
    try {
      const result = await invokeStudioAi({
        tool: toolSlug as StudioToolType,
        prompt: p,
      });
      setOutput(result);
    } catch (err) {
    ...
    <div>
      ...
      <div className="space-y-6">
        <div className="rounded-xl border border-border bg-card dark:border-white/5 dark:bg-card p-5">
          <PromptInput
            placeholder={
              PLACEHOLDERS[toolSlug] ?? "Describe what you want to generate…"
            }
            loading={loading}
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[200px_1fr]">
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-96 space-y-1 overflow-y-auto">
            {sessions.map((s) => (
              <div
                key={s.id}
                className="group flex items-center justify-between rounded-lg border border-border bg-card dark:border-white/5 dark:bg-card px-3 py-2 hover:border-brand-purple"
              >
                <Button
                  variant="ghost"
                  type="button"
                ...
                <Button
                  ...
                  className="ml-2 text-muted-foreground/80 opacity-0 hover:text-red-500 group-hover:opacity-100"
                >
                  <X className="h-3 w-3" />
                </Button>
              </div>
        ...
        <div className="flex flex-col rounded-xl border border-border bg-card dark:border-white/5 dark:bg-card">
          <div
            ref={scrollRef}
            className="max-h-[55vh] flex-1 space-y-4 overflow-y-auto p-4"
          >
          ...
          <div className="flex gap-2 border-t border-border p-3">
            ...
            <Button
              ...
              className="inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold hover:bg-primary/90 disabled:opacity-50"
            >
              Send
            </Button>
          </div>
