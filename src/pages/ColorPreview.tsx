...
export default function ColorPreview() {
  ...
  function addColor() {
    setColors([...colors, { name: `Color ${colors.length + 1}`, hex: '#cccccc' }]);
  }
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    setResult(null);
    try {
      const validColors = colors.filter((c) => isValidHexColor(normalizeHex(c.hex)));
      ...
      const res = await aiColorPreview({
        ...
        targetColors: validColors.map((c) => ({ name: c.name, hex: normalizeHex(c.hex) })),
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {!user && (
        <div className="mt-4 rounded-lg border p-4">
          <p className="text-sm text-muted-foreground">Sign in to use the AI Color Preview feature.</p>
        </div>
      )}
      ...
      {user && (
        <form onSubmit={handleGenerate} className="mt-6 space-y-6">
          {/* Image Upload */}
          <div>
            <label className="text-sm font-medium text-foreground">Room Photo (optional but recommended)</label>
            {imagePreview ? (
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground">Click to upload a room photo</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="text-sm font-medium text-foreground">Desired Mood</label>
          ...
          <div>
            <label className="text-sm font-medium text-foreground">Additional Description</label>
        ...
        <div className="mt-8 space-y-6">
            ...
            <div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <img src={imagePreview} alt="Before" className="w-full rounded-lg border" />
                </div>
                <div>
                  <p className="mb-2 text-xs font-medium text-muted-foreground">AI Color Suggestions</p>
                  <div className="flex h-full min-h-[200px] flex-col rounded-lg border">
                    {result.colorSuggestions.map((c, i) => (
                      <div key={i} className="flex items-center gap-3 border-b p-3 last:border-0">
                        <div className="h-12 w-12 rounded border" style={{ backgroundColor: c.hex }} />
                        <div>
          ...
          {result.previewDescription && (
            <div className="rounded-lg border p-6">
              <h3 className="text-lg font-semibold text-foreground">AI Visual Analysis</h3>
              <p className="mt-2 text-sm text-muted-foreground">{result.previewDescription}</p>
            </div>
          )}
          ...
          {result.colorSuggestions.length > 0 && (
            <div>
              <h3 className="mb-3 text-lg font-semibold text-foreground">Recommended Colors</h3>
              <div className="grid gap-4 sm:grid-cols-2">
                {result.colorSuggestions.map((c, i) => (
                  <div key={i} className="rounded-lg border p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-16 w-16 rounded-lg border" style={{ backgroundColor: c.hex }} />
                      <div>
                        <p className="font-medium text-foreground">{c.name}</p>
                        <p className="text-xs text-muted-foreground">{c.hex}</p>
                        <p className="text-xs font-medium text-brand-purple">{c.coverageArea}</p>
                      </div>
                    </div>
                    <p className="mt-3 text-sm text-muted-foreground">{c.reasoning}</p>
                  </div>
                ))}
              </div>
            </div>
