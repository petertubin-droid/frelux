...
function UsageBanner({ config, usage }: { config: AiAccessConfig; usage: AiUsageStatus }) {
...
function ChooseView({ onSelect, config }: { onSelect: (v: View) => void; config: AiAccessConfig | null }) {
  const disabled = config && (!config.aiEnabled || config.accessMode === 'disabled');
  return (
...
function AccessGate({ decision, config, onRewarded, isAuthenticated }: { decision: AiAccessDecision; config: AiAccessConfig | null; onRewarded: () => void; isAuthenticated: boolean }) {
          ...
          <div>
            ...
            {!isAuthenticated && (
              <p className="mt-3 text-xs text-neutral-500">
                <Link to="/login?redirect=/ai-color-assistant" className="font-semibold text-brand-purple hover:underline">Sign in or create an account</Link>
...
function TextConsultation({ config, usage, onUsageConsumed }: { config: AiAccessConfig | null; usage: AiUsageStatus | null; onUsageConsumed: () => void }) {
  const { user } = useAuth();
  ...
  const [matchedColor, setMatchedColor] = useState<DbColorCombination | null>(null);
  const [matchedPaintColors, setMatchedPaintColors] = useState<DbPaintColor[]>([]);
  const [rewardedState, setRewardedState] = useState<'idle' | 'requesting' | 'denied'>('idle');
  const submittedRef = useRef(false);

  useEffect(() => {
    fetchColorCombinations().then(({ data }) => setColors(data));
    fetchPaintColors({ pageSize: 500 }).then(({ data }) => setPaintColors(data));
  }, []);

  const decision: AiAccessDecision = config && usage ? checkAiAccess(config, usage) : { allowed: true, reason: 'free' };
  const canSubmit = description.trim().length >= 10 && status !== 'preparing' && status !== 'generating' && decision.allowed;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    logAnalyticsEvent('text_consultation_submitted', { length: description.length });
    ...
    try {
      ...
      onUsageConsumed();
      track('ai_recommendation_generated', { mode: 'text', colorCount: rec.colors.length });
      logAnalyticsEvent('ai_recommendation_generated', { mode: 'text', colorCount: rec.colors.length });
      setMatchedColor(findMatchingColor(rec, colors));
      setMatchedPaintColors(findMatchingPaintColors(rec, paintColors));
    } catch (err) {
      const message = err instanceof AiConsultError ? err.message : 'Something went wrong. Please try again.';
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-5">
      <div className="card p-6 sm:p-8 lg:col-span-3">
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          <label className="block">
            <span className="block text-sm font-semibold text-neutral-700">Your description</span>
            <textarea
              value={description}
              onChange={(e) => {
      ...
      <div className="lg:col-span-2">
        <ResultPanel status={status} error={error} recommendation={recommendation} matchedColor={matchedColor} matchedPaintColors={matchedPaintColors} mode="text" />
      </div>
    </div>
  );
}
...
function ImageConsultation({ config, usage, onUsageConsumed }: { config: AiAccessConfig | null; usage: AiUsageStatus | null; onUsageConsumed: () => void }) {
  const { user } = useAuth();
  ...
  const canSubmit = !!file && !!preview && status !== 'uploading' && status !== 'analyzing' && decision.allowed;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    track('image_analysis_started', { hasDescription: description.trim().length > 0 });
    logAnalyticsEvent('image_analysis_started', { hasDescription: description.trim().length > 0 });
    ...
    try {
      ...
      // Usage is consumed server-side by the edge function on success only.
      onUsageConsumed();
      track('ai_recommendation_generated', { mode: 'image', colorCount: rec.colors.length });
      logAnalyticsEvent('ai_recommendation_generated', { mode: 'image', colorCount: rec.colors.length });
      setMatchedColor(findMatchingColor(rec, colors));
      setMatchedPaintColors(findMatchingPaintColors(rec, paintColors));
    } catch (err) {
      const message = err instanceof AiConsultError ? err.message : 'Something went wrong. Please try again.';
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-5">
      <div className="card p-6 sm:p-8 lg:col-span-3">
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          ...
          <label className="mt-4 block">
            <span className="block text-sm font-semibold text-neutral-700">Add a description (optional)</span>
            <textarea
              value={description}
              onChange={(e) => {
                setDescription(e.target.value);
      ...
      <div className="lg:col-span-2">
        <ResultPanel status={status} error={error} recommendation={recommendation} matchedColor={matchedColor} matchedPaintColors={matchedPaintColors} mode="image" />
      </div>
    </div>
  );
}
...
function ImageDropzone({ onFile, disabled }: { onFile: (f: File | undefined) => void; disabled?: boolean }) {
  ...
  return (
    <div
      ...
      className={classNames(
        ...
        dragging ? 'border-brand-purple bg-brand-purple/5' : 'border-neutral-200 hover:border-neutral-300'
      )}
...
function ResultPanel({
  ...
  if (status === 'preparing' || status === 'generating' || status === 'uploading' || status === 'analyzing') {
    const label =
      status === 'uploading'
        ? 'Uploading image…'
        : status === 'analyzing'
    ...
    <div className="card sticky top-20 overflow-hidden">
      <div className="bg-brand-navy p-5 text-white">
        <div className="flex items-center gap-2 text-accent-green">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest">Your palette</span>
        </div>
        <p className="mt-2 text-sm text-white/80">{recommendation?.paletteSummary}</p>
      </div>
      ...
      {matchedColor && (
        <div className="border-t border-neutral-100 bg-brand-purple/5 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-brand-purple">Similar FRELUX color</p>
          <Link
          ...
          >
            <div className="flex gap-1">
              {[matchedColor.main_color_code, matchedColor.secondary_color_code, matchedColor.accent_color_code].map((hex) => (
                <div key={hex} className="h-8 w-8 rounded ring-1 ring-black/10" style={{ background: hex }} />
              ))}
            </div>
...
function findMatchingColor(rec: AiRecommendation, colors: DbColorCombination[]): DbColorCombination | null {
  ...
  for (const combo of colors) {
    const comboHexes = [combo.main_color_code, combo.secondary_color_code, combo.accent_color_code]
      .filter(Boolean)
...
function findMatchingPaintColors(rec: AiRecommendation, paintColors: DbPaintColor[]): DbPaintColor[] {
