...
export default function MarketplaceCategoryLocationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const cityFiltered = loc.city
        ? listingData.filter((l) => l.location_city === loc.city)
        : listingData;
  ...
  useSeo({
    ...
    structuredData: breadcrumbSchema([
      ...
      },
      {
        ...
        path: `/marketplace/${categorySlug}/${locationSlug}`,
      },
    ]),
  });
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground dark:text-muted-foreground">
            {category?.description ||
              `Find ${category?.name} professionals and jobs${locationLabel ? ` in ${locationLabel}` : ""}.`}
          </p>
        </div>
      </div>
            ...
            <section>
              ...
              {listings.length === 0 ? (
                <div className="rounded-xl border border-border/60 bg-card p-8 text-center dark:border-white/5 dark:bg-card">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    No active {category?.name} jobs in this location yet.
                  </p>
                  <Link
                    to="/marketplace/post/"
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {listings.map((listing) => (
                    ...
                    >
                      <h3 className="text-sm font-bold text-foreground dark:text-primary-foreground group-hover:text-brand-purple">
                        {listing.title}
                      </h3>
              ...
              {pros.length === 0 ? (
                <div className="rounded-xl border border-border/60 bg-card p-8 text-center dark:border-white/5 dark:bg-card">
                  <p className="text-sm text-muted-foreground">
                    No {category?.name} professionals listed in this location
                    yet.
                  </p>
                  <Link
                    to="/pro-connect/register/"
                    className="mt-4 inline-flex items-center gap-2 rounded-lg border border-brand-purple px-4 py-2 text-sm font-semibold text-brand-purple"
                  >
                    Register as a Pro
                  </Link>
                </div>
