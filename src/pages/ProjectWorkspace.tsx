...
export default function ProjectWorkspace() {
  useSeo({
    title: "Project Workspace: Manage Your Construction Projects",
    description:
      "Create and manage painting, screeding, tiling and finishing projects. Track progress, generate shopping lists, share estimates with clients.",
    canonicalPath: "/project-workspace",
    noIndex: true,
  });
  const { user } = useAuth();
  ...
  if (!user) {
    return (
      <div className="container mx-auto py-20 text-center text-muted-foreground">
        Please log in to access your project workspace.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader
        ...
        subtitle="Your central hub for managing all construction and finishing projects."
      />
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="mb-8 grid gap-4 grid-cols-2 sm:grid-cols-4">
          {[
            ...
            >
              <stat.icon
                className={`h-5 w-5 mb-2 ${stat.color} group-hover:scale-110 transition-transform`}
              />
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              className="w-full rounded-lg border pl-10 pr-4 py-2.5 text-sm bg-background transition-all duration-200 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none"
          ...
          <select
            className="rounded-lg border bg-background px-3 py-2.5 text-sm transition-all duration-200 focus:ring-2 focus:ring-primary/50 outline-none"
            value={statusFilter}
        ...
        {!loading && projects.length > 0 && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project, i) => (
              <div
                key={project.id}
                className="group rounded-xl border bg-card p-5 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1"
                style={{ animationDelay: `${i * 50}ms` }}
              >
                <div className="flex items-start justify-between mb-3">
                  <span
                    className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${STATUS_COLORS[project.status] || STATUS_COLORS.draft}`}
                  >
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mb-3">
                    ...
                    {project.client_address && (
                      <span className="inline-flex items-center gap-1">
                        <MapPin className="h-3 w-3" /> {project.client_address}
                      </span>
                    )}
                    <span className="inline-flex items-center gap-1">
