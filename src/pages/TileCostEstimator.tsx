...
export default function TileCostEstimator({
  ...
  useEffect(() => {
    if (passed.surfaceArea && passed.surfaceArea > 0 && !passed.input) {
      ...
      const rawResult = monitoredCalc("Tile Cost Estimator", () =>
        calculateTile(
          { ...inputWithArea, labourRatePerSqm: 0 },
          tileMaterials,
          currency,
          currencySymbol,
        ),
      ...
      const labourCost = calculateLabourCost(
        labourConfig,
        rawResult.surfaceArea,
      );
      const r: TileCalcResult = {
        ...rawResult,
        labourCost,
  ...
  function compute() {
    ...
    setResult(r);
    track("tile_estimate_generated", {
      surfaceType: input.surfaceType,
      total: r.grandTotal,
    });
    logAnalyticsEvent("tile_estimate_generated", {
      surfaceType: input.surfaceType,
      total: r.grandTotal,
    });
  }
