...
vi.mock('@/lib/supabase', () => {
  ...
  const mockFrom = vi.fn(() => {
    ...
    chain.single = vi.fn(() => Promise.resolve({ data: { id: 'test-id' }, error: null }));
    return chain;
  });
...
describe('Project Intelligence — Shopping List', () => {
  it('calculates shopping totals correctly with actual prices', () => {
    const items = [
      { id: '1', project_id: 'p1', category: 'paint', name: 'Satin', quantity: 4, unit: 'bucket', estimated_price: 15000, actual_price: 16000, total_price: 60000, supplier: null, notes: null, is_purchased: true, sort_order: 1 },
      { id: '2', project_id: 'p1', category: 'paint', name: 'Matt', quantity: 3, unit: 'bucket', estimated_price: 12000, actual_price: null, total_price: 36000, supplier: null, notes: null, is_purchased: false, sort_order: 2 },
    ];
  ...
  it('handles all items purchased with actual prices', () => {
    const items = [
      { id: '1', project_id: 'p1', category: 'paint', name: 'Satin', quantity: 2, unit: 'bucket', estimated_price: 10000, actual_price: 12000, total_price: 20000, supplier: 'Shop A', notes: null, is_purchased: true, sort_order: 1 },
    ];
    const totals = calculateShoppingTotals(items);
...
describe('Project Intelligence — Surface Assessment', () => {
  it('returns recommendations for new_wall', () => {
    ...
    expect(recs.some(r => r.recommendation.toLowerCase().includes('primer'))).toBe(true);
    expect(recs.some(r => r.recommendation.toLowerCase().includes('cure'))).toBe(true);
  });

  it('returns recommendations for previously_painted', () => {
    const recs = getSurfaceRecommendations('previously_painted');
    expect(recs.length).toBeGreaterThan(0);
    expect(recs.some(r => r.recommendation.toLowerCase().includes('clean'))).toBe(true);
  });
  ...
  it('returns recommendations for damp', () => {
    ...
    expect(recs.some(r => r.recommendation.toLowerCase().includes('moisture'))).toBe(true);
  });

  it('returns recommendations for cracked', () => {
    const recs = getSurfaceRecommendations('cracked');
    expect(recs.length).toBeGreaterThan(0);
    expect(recs.some(r => r.recommendation.toLowerCase().includes('fill'))).toBe(true);
  });

  it('returns recommendations for all 7 conditions', () => {
    const conditions = ['new_wall', 'previously_painted', 'smooth', 'rough', 'dirty', 'damp', 'cracked'];
  ...
  it('rough surface mentions increased paint absorption', () => {
    ...
    expect(recs.some(r => r.recommendation.toLowerCase().includes('30%'))).toBe(true);
  });
});

describe('Project Intelligence — AI Project Assistant', () => {
  it('analyzes a complete project description', () => {
    const input = 'I have a 3-bedroom house and want Matt in the bedrooms, Satin in the living room, and Tyrolene outside.';
    ...
    expect(result.recommendations.some(r => r.calculator_slug === 'paint-calculator')).toBe(true);
    expect(result.recommendations.some(r => r.calculator_slug === 'tyrolene-estimator')).toBe(true);
  ...
  it('does not fabricate quantities', () => {
    ...
    expect(JSON.stringify(result)).not.toMatch(/\d+\s*(bucket|litre|gallon|carton)/i);
  });

  it('recommends correct calculator for screeding', () => {
    const input = 'I need to screed my walls before painting';
    const result = analyzeProjectDescription(input);
    expect(result.recommendations.some(r => r.calculator_slug === 'screeding-calculator')).toBe(true);
  ...
  it('recommends correct calculator for tiling', () => {
    ...
    const result = analyzeProjectDescription(input);
    expect(result.recommendations.some(r => r.calculator_slug === 'tile-calculator')).toBe(true);
  ...
  it('recommends correct calculator for POP ceiling', () => {
    ...
    const result = analyzeProjectDescription(input);
    expect(result.recommendations.some(r => r.calculator_slug === 'pop-ceiling-calculator')).toBe(true);
...
describe('Project Intelligence — Progress Tracker', () => {
  ...
  it('calculates 50% with half completed', () => {
    const stages = [
      { id: '1', project_id: 'p1', stage_key: 'planning', stage_name: 'Planning', description: null, sort_order: 1, is_completed: true, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
      { id: '2', project_id: 'p1', stage_key: 'materials', stage_name: 'Materials', description: null, sort_order: 2, is_completed: false, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
      { id: '3', project_id: 'p1', stage_key: 'prep', stage_name: 'Prep', description: null, sort_order: 3, is_completed: true, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
      { id: '4', project_id: 'p1', stage_key: 'painting', stage_name: 'Painting', description: null, sort_order: 4, is_completed: false, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
    ];
  ...
  it('calculates 100% when all completed', () => {
    const stages = [
      { id: '1', project_id: 'p1', stage_key: 'a', stage_name: 'A', description: null, sort_order: 1, is_completed: true, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
      { id: '2', project_id: 'p1', stage_key: 'b', stage_name: 'B', description: null, sort_order: 2, is_completed: true, completed_at: null, notes: null, photo_url: null, created_at: '', updated_at: '' },
    ];
