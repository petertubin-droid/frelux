...
function buildPaintCalcMessage(data: PaintCalcShareData): string {
  ...
  lines.push('');
  lines.push('🔗 https://freluxtools.netlify.app/paint-calculator?mode=room-estimate');
...
function buildCostEstimateMessage(data: CostEstimateShareData): string {
  ...
  if (result.primerCost > 0) lines.push(`  • Primer: ${formatCurrency(result.primerCost, result.currencySymbol)}`);
  if (result.fillerCost > 0) lines.push(`  • Filler: ${formatCurrency(result.fillerCost, result.currencySymbol)}`);
  if (result.puttyCost > 0) lines.push(`  • Putty: ${formatCurrency(result.puttyCost, result.currencySymbol)}`);
  if (result.sandpaperCost > 0) lines.push(`  • Sandpaper: ${formatCurrency(result.sandpaperCost, result.currencySymbol)}`);
  if (result.brushesCost > 0) lines.push(`  • Brushes: ${formatCurrency(result.brushesCost, result.currencySymbol)}`);
  if (result.rollersCost > 0) lines.push(`  • Rollers: ${formatCurrency(result.rollersCost, result.currencySymbol)}`);
  if (result.otherMaterialsCost > 0) lines.push(`  • Other: ${formatCurrency(result.otherMaterialsCost, result.currencySymbol)}`);

  if (result.laborCost > 0) {
    lines.push('');
    lines.push(`*Labour:* ${formatCurrency(result.laborCost, result.currencySymbol)}`);
  }

  lines.push('');
  lines.push(`*GRAND TOTAL: ${formatCurrency(result.total, result.currencySymbol)}*`);
  lines.push('');
  lines.push('_Estimate only. Actual costs may vary._');
  lines.push('');
  lines.push('🔗 https://freluxtools.netlify.app/paint-calculator?mode=cost');
