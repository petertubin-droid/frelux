...
const { fetchProductCategories, fetchProductCategoryBySlug, searchProducts, fetchProduct, fetchMyProducts, createProduct, updateProduct, deleteProduct } = await import('./marketplace-products');
...
describe('marketplace-products (supabase not configured)', () => {
  ...
  it('searchProducts returns { products, total }', async () => {
    const result = await searchProducts({ query: 'paint' } as any);
  ...
  it('createProduct returns null (no data)', async () => {
    expect(await createProduct({} as any)).toBeNull();
  });
