...
const mockHeightAdjustmentRule: EstimationCalcRule = {
  ...
  rule_value: {
    ...
    message:
      "Wall height exceeds the FRELUX standard (7-8 ft). This is considered a high wall. Professional assessment recommended for non-standard heights.",
  },
  rule_status: "verified_frelux",
  description: "",
  is_active: true,
...
const mockOpeningDeductionRule: EstimationCalcRule = {
  id: "r6",
  rule_key: "opening_deduction_rule",
  calculator_type: "painting",
  rule_value: { deduction_percentage: 100 },
  rule_status: "verified_frelux",
  description: "",
  is_active: true,
  created_at: "",
...
const mockColourConditions: EstimationColourCondition[] = [
  ...
  {
    id: "cc3",
    condition_key: "dark_strong",
    name: "Dark / Strong Colour",
    description: null,
    requires_warning: true,
    min_coats_override: 3,
    is_active: true,
    sort_order: 4,
    created_at: "",
    updated_at: "",
  ...
  {
    id: "cc4",
    condition_key: "significant_transition",
    name: "Significant Colour Transition",
    description: null,
    requires_warning: true,
    min_coats_override: 3,
    is_active: true,
...
const mockSurfaceConditions: EstimationSurfaceCondition[] = [
  ...
  {
    id: "sc2",
    condition_key: "previously_painted_sound",
    name: "Previously Painted (Sound)",
    description: null,
    requires_preparation: false,
    primer_recommended: false,
    coverage_adjustment_factor: 1.0,
    is_active: true,
    sort_order: 2,
    created_at: "",
    updated_at: "",
  ...
  {
    id: "sc3",
    condition_key: "peeling_flaking",
    name: "Peeling / Flaking",
    description: null,
    requires_preparation: true,
    primer_recommended: true,
    coverage_adjustment_factor: 0.85,
    is_active: true,
    sort_order: 3,
    created_at: "",
    updated_at: "",
...
const mockProductionRules: ProductionRuleRow[] = [
  {
    product_category: "emulsion",
    quality_slug: null,
    location_rule: "owerri",
    min_quantity: 0,
    is_active: true,
  },
  {
  ...
  {
    product_category: "satin",
    quality_slug: "high_quality",
    location_rule: "outside_owerri",
    min_quantity: 5,
    is_active: true,
  },
...
function makeRoom(
  overrides: Partial<PaintingRoomInput> = {},
): PaintingRoomInput {
  return {
    room_id: "room-1",
    room_name: "Test Room",
  ...
  return {
    ...
    qualities:
      ...
      new Map([
        [
          "prod-emulsion",
          [mockQualityStandard, mockQualityPremium, mockQualityHigh],
        ],
      ]),
...
describe("FRELUX Painting Estimator", () => {
  ...
  describe("Area Calculations", () => {
    ...
    it("calculates opening area for multiple doors", () => {
      const area = calculateOpeningArea(
        [
  ...
  describe("Opening Deduction", () => {
    ...
    it("respects 50% deduction", () => {
      const halfRule: EstimationCalcRule = {
        ...mockOpeningDeductionRule,
        rule_value: { deduction_percentage: 50 },
      };
    ...
    it("deducts openings in room calculation", () => {
      ...
      expect(result.opening_deduction!.total_opening_area_m2).toBeGreaterThan(
        0,
      );
  ...
  describe("Height Adjustment", () => {
    ...
    it("does NOT flag 7 ft", () => {
      expect(
        evaluateHeightAdjustment(
          ...
          getStandardHeight(mockStandardHeightRule),
        ),
      ).toBeNull();
    });
    ...
    it("returns null when disabled", () => {
      const disabled: EstimationCalcRule = {
        ...mockHeightAdjustmentRule,
  ...
  describe("Quality & Coverage", () => {
    it("Standard > Premium > High Quality = decreasing litres (better coverage)", () => {
      const room = makeRoom();
      const std = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityStandard }),
      );
      const prem = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityPremium }),
      );
      const hq = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityHigh }),
      );
      expect(std.theoretical_wall_litres).toBeGreaterThan(
        prem.theoretical_wall_litres,
      );
      expect(prem.theoretical_wall_litres).toBeGreaterThan(
        hq.theoretical_wall_litres,
      );
    });
  });
  ...
  describe("Validation", () => {
    ...
    it("warns when openings unknown", () => {
      const r = validateRoomInput(
        makeRoom({ doors_unknown: true, windows_unknown: true }),
        mockProduct,
        mockQualityStandard,
      );
    ...
    it("reports error for missing coverage", () => {
      const result = calculateRoom(
        makeRoom(),
        makeRoomConfig({ quality: { ...mockQualityStandard, coverage: null } }),
      );
      expect(result.valid).toBe(false);
      expect(
        result.errors.some((e) =>
          e.includes("Coverage has not been configured"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Colour & Surface Conditions", () => {
    it("warns on dark/strong colour, no auto adjustment", () => {
      const result = calculateRoom(
        makeRoom({ colour_condition_key: "dark_strong" }),
        makeRoomConfig(),
      );
      expect(
        result.warnings.some((w) => w.includes("Strong colour transition")),
      ).toBe(true);
      const expected = calculateTheoreticalLitres(
    ...
    it("warns on peeling surface, recommends primer", () => {
      const result = calculateRoom(
        makeRoom({ surface_condition_key: "peeling_flaking" }),
        makeRoomConfig(),
      );
    ...
    it("primer not recommended for new plastered", () => {
      const result = calculateRoom(
        makeRoom({ surface_condition_key: "new_plastered" }),
        makeRoomConfig(),
      );
  ...
  describe("Customer Summary", () => {
    it("produces customer-facing summary in painter language", () => {
      const result = calculateRoom(
        makeRoom({ room_name: "Living Room", include_ceiling: true }),
        makeRoomConfig(),
      );
  ...
  describe("TEST 1: 10×12 ft room, 8ft, 2 coats, walls only", () => {
    it("produces approximately 1 practical bucket", () => {
      ...
      expect(result.practical_total_buckets).toBe(
        result.practical_wall_buckets,
      );
  ...
  describe("TEST 2: 12×12 ft room, 8ft, 2 coats, ceiling included", () => {
    it("calculates wall + ceiling with theoretical and practical buckets", () => {
      ...
      expect(result.theoretical_total_buckets).toBe(
        result.theoretical_wall_buckets + 0.5,
      );
      expect(result.practical_total_buckets).toBeGreaterThanOrEqual(
        result.theoretical_total_buckets,
      );
  ...
  describe("TEST 3: 9 ft height triggers high-height rule", () => {
    it("flags 9 ft as too high with warning", () => {
      ...
      expect(result.warnings.some((w) => w.includes("FRELUX standard"))).toBe(
        true,
      );
  ...
  describe("TEST 4: Multi-room project", () => {
    it("calculates each room independently and aggregates", () => {
      ...
      const result = calculatePaintingProject(
        {
          ...
          user_id: null,
          client_hash: null,
          project_description: "3-room project",
      ...
      expect(result.rooms[0].theoretical_wall_litres).not.toBe(
        result.rooms[1].theoretical_wall_litres,
      );
      expect(result.rooms[1].theoretical_wall_litres).not.toBe(
        result.rooms[2].theoretical_wall_litres,
      );
  ...
  describe("TEST 5: Quality change affects coverage", () => {
    it("Premium requires fewer litres than Standard (better coverage)", () => {
      ...
      const stdResult = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityStandard }),
      );
      const premResult = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityPremium }),
      );

      expect(premResult.theoretical_wall_litres).toBeLessThan(
        stdResult.theoretical_wall_litres,
      );
      expect(premResult.coverage_m2_per_liter).toBeGreaterThan(
        stdResult.coverage_m2_per_liter!,
      );
    });
    ...
    it("High Quality requires even fewer litres", () => {
      ...
      const hqResult = calculateRoom(
        room,
        makeRoomConfig({ quality: mockQualityHigh }),
      );

      expect(hqResult.theoretical_wall_litres).toBeLessThan(
        premResult.theoretical_wall_litres,
      );
    });
  });
  ...
  describe("TEST 6: Price snapshot", () => {
    it("new estimate uses current active price", () => {
      ...
      expect(newResult.total_material_cost).toBeGreaterThan(
  ...
  describe("TEST 7: Labour exclusion", () => {
    ...
    it("no labour line items in result", () => {
      ...
      const labourItems = result.line_items.filter(
        (i) => i.item_type === "labour",
      );
    ...
    it("customer summary does not mention labour cost", () => {
      ...
      expect(result.customer_summary.material_cost_formatted).not.toContain(
        "labour",
      );
      expect(result.customer_summary.material_cost_formatted).not.toContain(
        "labor",
  ...
  describe("TEST 8: No m² input required", () => {
    ...
    it("customer summary shows room dimensions, not m² as primary", () => {
      const result = calculateRoom(
        makeRoom({ room_name: "Living Room" }),
        makeRoomConfig(),
      );
      const cs = result.customer_summary;
      ...
      const result = calculatePaintingProject(
        ...
        makeProjectConfig({
          ...
          prices: new Map([
            ...
            [
              "qual-matt-prem",
              { ...mockPrice, id: "pm", ref_id: "qual-matt-prem" },
            ],
          ]),
        }),
      );
