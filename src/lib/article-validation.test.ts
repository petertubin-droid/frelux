...
describe('Article Validation Engine', () => {
  ...
  describe('slug-format', () => {
    it('passes with valid kebab-case slug', () => {
      const result = validateArticle(makeValidArticle({ slug: 'my-article-title' }));
  ...
  describe('title-length', () => {
    it('passes with title between 10-70 chars', () => {
      const result = validateArticle(makeValidArticle({ title: 'A Good Article Title' }));
  ...
  describe('meta-title-length', () => {
    it('passes with meta title between 30-60 chars', () => {
      const result = validateArticle(makeValidArticle({ meta_title: 'Complete Guide to Painting Interior Walls' }));
      const rule = result.rules.find((r) => r.rule === 'meta-title-length')!;
      expect(rule.passed).toBe(true);
    });

    it('warns with meta title shorter than 30 chars', () => {
      const result = validateArticle(makeValidArticle({ meta_title: 'Paint Guide' }));
      const rule = result.rules.find((r) => r.rule === 'meta-title-length')!;
      expect(rule.passed).toBe(false);
    ...
    it('warns with meta title longer than 60 chars', () => {
      const result = validateArticle(makeValidArticle({ meta_title: 'A'.repeat(61) }));
      const rule = result.rules.find((r) => r.rule === 'meta-title-length')!;
      expect(rule.passed).toBe(false);
      expect(rule.level).toBe('warning');
    });
  ...
  describe('meta-description-length', () => {
    it('passes with meta description between 120-160 chars', () => {
      const desc = 'Step by step guide to painting interior walls like a pro. Covers tools, preparation, priming, cutting in, rolling, and pro tips for a flawless finish.';
      const result = validateArticle(makeValidArticle({ meta_description: desc }));
      const rule = result.rules.find((r) => r.rule === 'meta-description-length')!;
      expect(rule.passed).toBe(true);
    });

    it('errors when meta description is null', () => {
      const result = validateArticle(makeValidArticle({ meta_description: null }));
      const rule = result.rules.find((r) => r.rule === 'meta-description-missing')!;
      expect(rule.passed).toBe(false);
    ...
    it('warns when meta description is too short', () => {
      const result = validateArticle(makeValidArticle({ meta_description: 'Short description.' }));
      const rule = result.rules.find((r) => r.rule === 'meta-description-length')!;
      expect(rule.passed).toBe(false);
      expect(rule.level).toBe('warning');
    });
    ...
    it('warns when meta description is too long', () => {
      const result = validateArticle(makeValidArticle({ meta_description: 'A'.repeat(161) }));
      const rule = result.rules.find((r) => r.rule === 'meta-description-length')!;
      expect(rule.passed).toBe(false);
      expect(rule.level).toBe('warning');
    });
  ...
  describe('word-count', () => {
    ...
    it('passes recommended with 1500+ words', () => {
      const content = '## Intro\n\n' + 'word '.repeat(1600) + '\n\n## Conclusion\n\n' + 'word '.repeat(10);
      const result = validateArticle(makeValidArticle({ content }));
      const rule = result.rules.find((r) => r.rule === 'word-count-recommended')!;
      expect(rule.passed).toBe(true);
    });
  ...
  describe('heading structure', () => {
    ...
    it('passes with zero H1 headings in content', () => {
      const content = 'Just a paragraph with no headings at all. ' + 'word '.repeat(350) + '\n\n## Subheading\n\nMore content here.';
      const result = validateArticle(makeValidArticle({ content }));
  ...
  describe('author-attribution', () => {
    it('passes with author present', () => {
      const result = validateArticle(makeValidArticle({ author: 'Frelux Editorial Team' }));
  ...
  describe('no-placeholder-content', () => {
    it('passes with real content', () => {
      const result = validateArticle(makeValidArticle());
      const rule = result.rules.find((r) => r.rule === 'no-placeholder-content')!;
      expect(rule.passed).toBe(true);
    });

    it('fails with lorem ipsum', () => {
      const result = validateArticle(makeValidArticle({ content: makeThinContent() }));
      const rule = result.rules.find((r) => r.rule === 'no-placeholder-content')!;
      expect(rule.passed).toBe(false);
      expect(rule.level).toBe('error');
    });
    ...
    it('fails with [insert content] placeholder', () => {
      const result = validateArticle(makeValidArticle({ content: '## Intro\n\n[insert content here]\n\n## Conclusion\n\nDone.' }));
      const rule = result.rules.find((r) => r.rule === 'no-placeholder-content')!;
      expect(rule.passed).toBe(false);
    });
  ...
  describe('internal-links', () => {
    ...
    it('warns with no internal links', () => {
      const result = validateArticle(makeValidArticle({ content: makeLongContent() }));
      const rule = result.rules.find((r) => r.rule === 'internal-links')!;
      expect(rule.passed).toBe(false);
      expect(rule.level).toBe('warning');
    });
  ...
  describe('keyword-density', () => {
    it('detects keyword stuffing', () => {
      ...
      const content = 'paint painting paint painting paint '.repeat(100) + ' word '.repeat(200) + '\n\n## Conclusion\n\nDone.';
      const result = validateArticle(makeValidArticle({
        content,
        meta_keywords: 'paint painting',
      }));
      const densityRule = result.rules.find((r) => r.rule === 'keyword-density' && !r.passed);
      expect(densityRule).toBeDefined();
    });

    it('passes with reasonable keyword density', () => {
      const result = validateArticle(makeValidArticle());
      const densityRules = result.rules.filter((r) => r.rule === 'keyword-density' && !r.passed);
  ...
  describe('read-time-accuracy', () => {
    it('passes with accurate read time', () => {
      // ~407 words / 200 ≈ 2 min
      const result = validateArticle(makeValidArticle({ read_time_minutes: 2 }));
    ...
    it('warns with inaccurate read time', () => {
      ...
      const result = validateArticle(makeValidArticle({ read_time_minutes: 20 }));
      const rule = result.rules.find((r) => r.rule === 'read-time-accuracy');
      if (rule) {
        expect(rule.passed).toBe(false);
        expect(rule.level).toBe('warning');
  ...
  describe('category-assigned', () => {
    it('passes with category slug', () => {
      const result = validateArticle(makeValidArticle({ category_slug: 'painting-guides' }));
  ...
  describe('isArticleCompliant', () => {
    ...
    it('returns false for article with errors', () => {
      expect(isArticleCompliant(makeValidArticle({ author: null }))).toBe(false);
      expect(isArticleCompliant(makeValidArticle({ meta_description: null }))).toBe(false);
      expect(isArticleCompliant(makeValidArticle({ slug: 'Invalid Slug' }))).toBe(false);
    });
  });
  ...
  describe('formatValidationIssues', () => {
    ...
    it('returns issue messages for non-compliant article', () => {
      const result = validateArticle(makeValidArticle({ author: null, meta_description: null }));
      const issues = formatValidationIssues(result);
      expect(issues.length).toBeGreaterThan(0);
      expect(issues.some((i) => i.includes('author-attribution'))).toBe(true);
      expect(issues.some((i) => i.includes('meta-description-missing'))).toBe(true);
    });

    it('includes emoji prefixes for issue levels', () => {
      const result = validateArticle(makeValidArticle({ excerpt: null, author: null }));
      const issues = formatValidationIssues(result);
    ...
    const realArticles: { name: string; article: ArticleInput }[] = [
      {
        ...
        article: makeValidArticle({
          ...
          meta_description:
            'Step by step guide to painting interior walls like a pro. Covers tools, preparation, priming, cutting in, rolling, second coats, common mistakes, and pro tips for a flawless finish.',
          meta_keywords: 'interior wall painting, painting guide, how to paint walls, painting techniques, professional painting tips, wall preparation, primer, cutting in, rolling paint',
          category_slug: 'painting-guides',
          read_time_minutes: 12,
        }),
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'paint kitchen cabinets, cabinet painting, DIY kitchen, cabinet refinishing, painting without removing',
          category_slug: 'diy-tutorials',
          read_time_minutes: 11,
        }),
      },
      {
        name: 'paint-buying-guides',
        article: makeValidArticle({
          ...
          meta_keywords: 'paint types, paint finish, emulsion, satin, matte, gloss, paint buying guide, choosing paint',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'color psychology, paint colors, mood, space perception, color theory, room colors',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'surface preparation, paint prep, wall preparation, sanding, priming, crack repair, surface cleaning',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'painting tips, painting techniques, professional painting, cutting in, rolling, paint finish, pro tips',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'paint FAQ, paint questions, color FAQ, paint calculator FAQ, painting help',
          category_slug: 'faqs',
          read_time_minutes: 10,
        }),
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'paint brands, paint comparison, premium paint, paint reviews, best paint Nigeria, paint quality',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'painting videos, video tutorials, painting walkthroughs, beginner painting, learn to paint',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'paint industry trends, paint innovations, eco-friendly paint, smart paint, 2026 trends',
      ...
      },
      {
        ...
        article: makeValidArticle({
          ...
          meta_keywords: 'painting case studies, painting projects, before and after, painting transformations, real projects',
