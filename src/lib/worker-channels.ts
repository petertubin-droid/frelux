...
export async function fetchChannelCategories(): Promise<
  DbWorkerChannelCategory[]
> {
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] fetchChannelCategories:", error.message);
    return [];
  }
...
export async function fetchChannels(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] fetchChannels:", error.message);
    return [];
  }
  ...
  if (userId) {
    ...
    joinedChannelIds = new Set(
      (memberships ?? []).map((m: { channel_id: string }) => m.channel_id),
    );
  }
  ...
  return (data ?? []).map((ch: Record<string, unknown>) => ({
    ...
    member_count:
      (ch.member_count as { count: number }[] | undefined)?.[0]?.count ?? 0,
...
export async function fetchChannelBySlug(
  ...
  if (error || !data) {
    if (import.meta.env.DEV && error)
      console.error("[worker-channels] fetchChannelBySlug:", error.message);
    return null;
  }
...
export async function joinChannel(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] joinChannel:", error.message);
    return false;
  }
  return true;
}
...
export async function leaveChannel(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] leaveChannel:", error.message);
    return false;
  }
  return true;
}
...
export async function fetchMessages(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] fetchMessages:", error.message);
    return [];
  }
...
export async function sendMessage(
...
): Promise<DbWorkerChannelMessage | null> {
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] sendMessage:", error.message);
    return null;
  }
...
export async function deleteMessage(messageId: string): Promise<boolean> {
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] deleteMessage:", error.message);
    return false;
  }
  return true;
}
...
export async function toggleReaction(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] toggleReaction:", error.message);
    return false;
  }
  return true;
}
...
export async function fetchModerationConfig(): Promise<DbWorkerModerationConfig | null> {
  ...
  if (error || !data) {
    if (import.meta.env.DEV && error)
      console.error("[worker-channels] fetchModerationConfig:", error.message);
    return null;
  }
...
export async function fetchPriceUpdateSummaries(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error(
        "[worker-channels] fetchPriceUpdateSummaries:",
        error.message,
      );
    return [];
  }
...
export async function getUserVerificationTier(userId: string): Promise<number> {
  const { data, error } = await supabase.rpc("get_user_verification_tier", {
    p_user_id: userId,
  });
  if (error) {
    if (import.meta.env.DEV)
      console.error(
        "[worker-channels] getUserVerificationTier:",
        error.message,
      );
    return 0;
  }
...
export async function fetchChatUserProfile(
  ...
  if (error || !data) {
    if (import.meta.env.DEV && error)
      console.error("[worker-channels] fetchChatUserProfile:", error.message);
    return null;
  }
...
export async function sendMobileOTP(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] sendMobileOTP:", error.message);
...
export async function verifyMobileOTP(
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] verifyMobileOTP:", error.message);
    return { success: false, message: error.message };
  }
...
export async function submitNIN(
  ninNumber: string,
  userId: string,
): Promise<{ success: boolean; message: string }> {
  ...
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] submitNIN:", error.message);
    return { success: false, message: error.message };
  }
  const result = data as { success: boolean; message: string } | null;
  return {
...
export async function adminGetNinSubmissions(
  status: string = "pending",
): Promise<NinSubmission[]> {
  const { data, error } = await supabase.rpc("admin_get_nin_submissions", {
    p_status: status,
  });
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] adminGetNinSubmissions:", error.message);
    return [];
  }
  return (data ?? []) as NinSubmission[];
}

export async function adminApproveNin(
  profileId: string,
): Promise<{ success: boolean; message: string }> {
  const {
    data: { user },
...
export async function adminRejectNin(
  profileId: string,
  reason: string,
): Promise<{ success: boolean; message: string }> {
  const {
...
export async function adminGetNinHistory(
  profileId: string,
): Promise<NinHistoryEntry[]> {
  const { data, error } = await supabase.rpc("admin_get_nin_history", {
    p_profile_id: profileId,
  });
  if (error) {
    if (import.meta.env.DEV)
      console.error("[worker-channels] adminGetNinHistory:", error.message);
    return [];
  }
