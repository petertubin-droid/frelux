import { resolveSurfaceFactor, resolveMinCoats } from "./calc";
...
describe("calculatePaintableArea", () => {
  it("subtracts openings from surface", () => {
    ...
    expect(calculatePaintableArea(60, 24, 3.84, 4.32, true)).toBeCloseTo(
      75.84,
      2,
    );
  });
  ...
  it("excludes ceiling when includeCeiling is false", () => {
    // 60 - 3.84 - 4.32 = 51.84
    expect(calculatePaintableArea(60, 24, 3.84, 4.32, false)).toBeCloseTo(
      51.84,
      2,
    );
  });
  ...
  it("calculates leftover from container rounding", () => {
    ...
    if (result.totalRecommendedLiters > result.adjustedLiters) {
      ...
      expect(result.leftoverLiters).toBeCloseTo(
        Math.round(
          (result.totalRecommendedLiters - result.adjustedLiters) * 100,
        ) / 100,
        1,
      );
    }
...
describe("height warning", () => {
  it("warns when wall height exceeds 8ft standard", () => {
    const input: CalculatorInput = {
      ...
      length: 12,
      width: 12,
      wallHeight: 12, // 12 feet > 8 feet standard
      doors: 0,
      doorDims: { width: 0.8, height: 2.4 },
      windows: 0,
      windowDims: { width: 1.2, height: 1.2 },
...
// ─────────────────────────────────────────────────────────
// Configurable rule resolution (audit patch cf_calc_hardcoded_fallbacks)
// Precedence: per-condition DB override → scalar DB override →
// documented default. Invalid configured values fall back safely.
// ─────────────────────────────────────────────────────────
describe("configurable surface factor resolution", () => {
  it("uses the documented default with no config", () => {
    expect(resolveSurfaceFactor("textured")).toBe(0.85);
    expect(resolveSurfaceFactor("rough")).toBe(0.75);
    expect(resolveSurfaceFactor("new_plaster")).toBe(0.8);
    expect(resolveSurfaceFactor(undefined)).toBe(1.0);
  });

  it("per-condition DB override beats the documented default", () => {
    expect(
      resolveSurfaceFactor("textured", {
        surfaceFactorOverrides: { textured: 0.9 },
      }),
    ).toBe(0.9);
  });

  it("per-condition override beats scalar override", () => {
    expect(
      resolveSurfaceFactor("textured", {
        surfaceFactorOverride: 0.95,
        surfaceFactorOverrides: { textured: 0.9 },
      }),
    ).toBe(0.9);
  });

  it("scalar DB override beats the documented default", () => {
    expect(resolveSurfaceFactor("rough", { surfaceFactorOverride: 0.7 })).toBe(
      0.7,
    );
  });
  ...
  it("rejects invalid configured factors — falls back to documented", () => {
    ...
    expect(resolveSurfaceFactor("rough", { surfaceFactorOverride: 1.5 })).toBe(
      0.75,
    );
  });
});

describe("configurable min-coats resolution", () => {
  it("uses the documented default with no config", () => {
    expect(resolveMinCoats("same_or_light")).toBe(2);
    expect(resolveMinCoats("dark_over_light")).toBe(3);
  });

  it("per-condition DB override beats the documented default", () => {
    expect(
      resolveMinCoats("dark_over_light", {
        minCoatsOverrides: { dark_over_light: 4 },
      }),
    ).toBe(4);
  });
  ...
  it("rejects invalid configured coats — falls back to documented", () => {
    ...
    expect(resolveMinCoats("same_or_light", { minCoatsOverride: 1.5 })).toBe(2);
  });
});
