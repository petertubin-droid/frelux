...
describe('Market Profile Creation', () => {
  ...
  it('Nigeria has package sizes', () => {
    const profile = createNigeriaProfile();
    expect(profile.defaultPackageSizes.paint).toEqual({ size: 20, unit: 'litres' });
    expect(profile.defaultPackageSizes.cement).toEqual({ size: 50, unit: 'kg' });
    expect(profile.defaultPackageSizes.tiles).toEqual({ size: 1.44, unit: 'm2' });
  });
