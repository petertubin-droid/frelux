...
describe('Volume Units', () => {
  ...
  describe('convertVolume', () => {
    it('converts litres to cubic feet', () => {
      ...
      expect(convertVolume(1000, 'litres', 'cubic_feet')).toBeCloseTo(35.3147, 2);
    });
...
describe('Calculator Unit Support', () => {
  ...
  it('all calculator contexts have declarations', () => {
    const contexts = ['painting', 'screeding', 'tiling', 'grafitex', 'block', 'pop', 'tyrolene', 'fence_screeding', 'fence_painting'] as const;
    for (const ctx of contexts) {
      expect(CALCULATOR_UNIT_SUPPORT[ctx]).toBeDefined();
      expect(CALCULATOR_UNIT_SUPPORT[ctx].lengthUnits.length).toBeGreaterThan(0);
      expect(CALCULATOR_UNIT_SUPPORT[ctx].quantityUnits.length).toBeGreaterThan(0);
    }
  });
});
