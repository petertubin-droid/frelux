...
describe('Feature 22: Shared Engine Integration', () => {
  describe('Pipeline Sources', () => {
    it('has both source labels', () => {
      expect(PIPELINE_SOURCE_LABELS.building_to_roof).toBe('Building-to-Roof Estimator');
      expect(PIPELINE_SOURCE_LABELS.ai_image_estimator).toBe('AI Image Estimator');
    });
  ...
  describe('Pipeline Stages', () => {
    it('has all 8 pipeline stages', () => {
      const stages: PipelineStage[] = [
        'measurement', 'space_element', 'calculation',
        'material', 'market_intelligence', 'validation',
        'estimate', 'report',
      ];
  ...
  describe('Building-to-Roof Pipeline', () => {
    ...
    it('includes roof calculations when roof sections provided', () => {
      ...
      const result = executeSharedPipeline(input);
      const roofCalc = result.stages.calculation.results.find((r) => r.calculator === 'Roof Area');
    ...
    it('marks manual input as verified', () => {
      ...
      expect(result.stages.validation.verificationSummary.verified).toBeGreaterThan(0);
    });
  });
  ...
  describe('AI Image Estimator Pipeline', () => {
    ...
    it('flags scale issues in validation', () => {
      ...
      const result = executeSharedPipeline(input);
      expect(result.stages.validation.issues).toContain('Scale not verified — AI dimensions may be inaccurate.');
    });
    ...
    it('flags review-required in validation', () => {
      ...
      expect(result.stages.validation.issues).toContain('Measurement data requires user review before final use.');
    });
  });
  ...
  describe('Pipeline Output', () => {
    ...
    it('reports no verified prices when market intelligence has none', () => {
      ...
      expect(result.stages.estimate.notes).toContain('No verified prices — estimate shown without pricing');
    });
  });
  ...
  describe('No Duplicate Systems', () => {
    it('uses the same pipeline for both sources', () => {
      ...
      expect(Object.keys(btrResult.stages)).toEqual(Object.keys(aiResult.stages));
