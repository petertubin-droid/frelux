...
describe('Feature 19: Labour Engine', () => {
  describe('buildLabourLineItem', () => {
    it('builds per-unit rate line item', () => {
      const input: LabourActivityInput = { activity: 'Plastering', trade: 'plastering', quantity: 100, unit: 'm²' };
      const rate: LabourRate = { activity: 'Plastering', trade: 'plastering', rateType: 'per_unit', ratePerUnit: 500, unit: 'm²', currency: 'NGN', source: 'admin_config', overridden: false };
    ...
    it('builds lump sum line item', () => {
      const input: LabourActivityInput = { activity: 'Site setup', trade: 'site_preparation', quantity: 1, unit: 'project' };
      const rate: LabourRate = { activity: 'Site setup', trade: 'site_preparation', rateType: 'lump_sum', ratePerUnit: 0, unit: 'project', lumpSumAmount: 50000, currency: 'NGN', source: 'admin_config', overridden: false };
      const item = buildLabourLineItem(input, rate);
      expect(item.lineTotal).toBe(50000);
    });

    it('builds percentage-of-materials line item', () => {
      const input: LabourActivityInput = { activity: 'Electrical', trade: 'electrical', quantity: 1, unit: 'project', materialCostForPercentage: 200000 };
      const rate: LabourRate = { activity: 'Electrical', trade: 'electrical', rateType: 'percentage_of_materials', ratePerUnit: 0, unit: 'project', percentage: 15, currency: 'NGN', source: 'admin_config', overridden: false };
      const item = buildLabourLineItem(input, rate);
  ...
  describe('calculateLabourCost', () => {
    it('calculates total with trade subtotals', () => {
      const activities: LabourActivityInput[] = [
        { activity: 'Plastering', trade: 'plastering', quantity: 100, unit: 'm²' },
        { activity: 'Block laying', trade: 'masonry', quantity: 200, unit: 'm²' },
      ];
      const rates = new Map<string, LabourRate>([
        { activity: 'Plastering', trade: 'plastering', rateType: 'per_unit', ratePerUnit: 500, unit: 'm²', currency: 'NGN', source: 'admin_config', overridden: false },
        { activity: 'Block laying', trade: 'masonry', rateType: 'per_unit', ratePerUnit: 800, unit: 'm²', currency: 'NGN', source: 'admin_config', overridden: false },
      ].map((r) => [r.activity, r]));
    ...
    it('flags missing rates', () => {
      const activities: LabourActivityInput[] = [
        { activity: 'Plastering', trade: 'plastering', quantity: 100, unit: 'm²' },
        { activity: 'Unknown', trade: 'other', quantity: 10, unit: 'm²' },
      ];
      const rates = new Map<string, LabourRate>([
        { activity: 'Plastering', trade: 'plastering', rateType: 'per_unit', ratePerUnit: 500, unit: 'm²', currency: 'NGN', source: 'admin_config', overridden: false },
      ].map((r) => [r.activity, r]));
