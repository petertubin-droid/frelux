...
describe("Feature 19: Confidence / Verification States", () => {
  describe("Verification States", () => {
    ...
    it("marks verified states correctly", () => {
      ...
      expect(VERIFICATION_STATE_VERIFIED.ai_detected_review_required).toBe(
        false,
      );
    ...
    it("flags states requiring action", () => {
      expect(
        VERIFICATION_STATE_REQUIRES_ACTION.ai_detected_review_required,
      ).toBe(true);
  ...
  describe("State Transitions", () => {
    ...
    it("allows ai_detected → ai_detected_review_required", () => {
      expect(canTransition("ai_detected", "ai_detected_review_required")).toBe(
        true,
      );
    });
    ...
    it("allows ai_detected_review_required → user_verified", () => {
      expect(
        canTransition("ai_detected_review_required", "user_verified"),
      ).toBe(true);
    });
    ...
    it("returns original state on invalid transition", () => {
      const result = transitionVerificationState(
        "market_price_verified",
        "ai_detected",
      );
  ...
  describe("Unified Confidence Assessment", () => {
    ...
    it("provides dimension breakdown for UI", () => {
      ...
      expect(assessment.dimensionBreakdown.aiDetection).toContain(
        "High AI Confidence",
      );
      expect(assessment.dimensionBreakdown.aiDetection).toContain(
        "does NOT guarantee",
      ...
      expect(assessment.dimensionBreakdown.marketPrice).toContain(
        "No market price",
      );
    });
  ...
  describe("Verification Record Lifecycle", () => {
    ...
    it("transitions through lifecycle: ai_detected → review → verified", () => {
      ...
      record = updateVerificationRecord(record, "user_verified", {
        reviewNotes: "Checked against local market",
      });
