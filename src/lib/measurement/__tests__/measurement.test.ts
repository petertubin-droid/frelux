...
describe('Different Room Types', () => {
  test('bedroom and kitchen with different dimensions remain separate', () => {
    ...
    project.sections = [
      createMeasurementSection('Bedrooms', [createMeasurementGroup('12×12 ft Bedroom ×2', bedroomEntry)]),
      createMeasurementSection('Kitchen', [createMeasurementGroup('10×10 ft Kitchen', kitchenEntry)]),
    ];

    const result = calculateMeasurementProject(project);
    expect(result.sectionResults.length).toBe(2);
    expect(result.sectionResults[0].totalAreaM2).not.toBeCloseTo(result.sectionResults[1].totalAreaM2, 1);
    expect(result.totalAreaM2).toBeCloseTo(
      result.sectionResults[0].totalAreaM2 + result.sectionResults[1].totalAreaM2,
      6,
    );
  });
});
...
describe('Fence Calculation', () => {
  test('fence with multiple dimensions, different partitions', () => {
    ...
    expect(result.sectionResults[0].groupResults[0].totalAreaM2).toBeCloseTo(d1Expected, 4);
    expect(result.sectionResults[0].groupResults[1].totalAreaM2).toBeCloseTo(d2Expected, 4);
  });
...
describe('Grafitex Calculation', () => {
  ...
  test('multiple grafitex surfaces aggregate correctly', () => {
    ...
    const surfaces = [
      createMeasurementEntry({ length: 40, height: 10, unit: 'feet', quantity: 1, surfaceType: 'exterior', description: 'Front wall' }),
      createMeasurementEntry({ length: 40, height: 10, unit: 'feet', quantity: 1, surfaceType: 'exterior', description: 'Rear wall' }),
      createMeasurementEntry({ length: 30, height: 10, unit: 'feet', quantity: 1, surfaceType: 'exterior', description: 'Left wall' }),
      createMeasurementEntry({ length: 30, height: 10, unit: 'feet', quantity: 1, surfaceType: 'exterior', description: 'Right wall' }),
    ];

    project.sections = [
      createMeasurementSection('Exterior Surfaces', surfaces.map((s, i) =>
...
describe('Validation', () => {
  ...
  test('rejects inches in painting context', () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.toLowerCase().includes('inches'))).toBe(true);
  });
