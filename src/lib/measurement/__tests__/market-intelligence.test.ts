...
describe('Price Lookup', () => {
  function setup() {
    return addPrices(createMarketPriceCatalog(), [
      createMarketPrice({ productName: 'Cement', brand: 'Lafarge', marketCode: 'NG', price: 7500, isApproved: true, recordedDate: '2026-08-20T10:00:00Z' }),
      createMarketPrice({ productName: 'Cement', brand: 'Dangote', marketCode: 'NG', price: 7000, isApproved: true, recordedDate: '2026-08-22T10:00:00Z' }),
      createMarketPrice({ productName: 'Cement', brand: 'Lafarge', marketCode: 'GH', price: 95, isApproved: true, recordedDate: '2026-08-15T10:00:00Z' }),
      createMarketPrice({ productName: 'Paint', brand: 'Dulux', marketCode: 'NG', price: 25000, isApproved: false, recordedDate: '2026-08-10T10:00:00Z' }),
    ]);
  }
  ...
  it('filters by brand', () => {
    const catalog = setup();
    const prices = findPrices(catalog, { productName: 'Cement', brand: 'Lafarge' });
    expect(prices.length).toBe(2);
  });

  it('filters by market', () => {
    const catalog = setup();
    const prices = findPrices(catalog, { productName: 'Cement', marketCode: 'GH' });
  ...
  it('sorts by date (most recent first)', () => {
    const catalog = setup();
    const prices = findPrices(catalog, { productName: 'Cement', marketCode: 'NG' });
...
describe('Price Freshness', () => {
  it('calculates age in days', () => {
    const oldPrice = createMarketPrice({ recordedDate: '2026-01-01T00:00:00Z' });
    const age = getPriceAgeDays(oldPrice);
    expect(age).toBeGreaterThan(200);
  });

  it('checks if price is fresh', () => {
    const recentPrice = createMarketPrice({ recordedDate: new Date().toISOString() });
    const oldPrice = createMarketPrice({ recordedDate: '2026-01-01T00:00:00Z' });
  ...
  it('gets freshness label', () => {
    const recent = createMarketPrice({ recordedDate: new Date().toISOString() });
...
describe('Price Estimates', () => {
  ...
  it('builds multiple estimates from catalog', () => {
    let catalog = createMarketPriceCatalog();
    catalog = addPrice(catalog, createMarketPrice({ productName: 'Cement', price: 7500, isApproved: true, packageUnit: 'bags' }));
    catalog = addPrice(catalog, createMarketPrice({ productName: 'Paint', price: 25000, isApproved: true, packageUnit: 'buckets' }));
  ...
  it('returns empty for unapproved prices', () => {
    let catalog = createMarketPriceCatalog();
    catalog = addPrice(catalog, createMarketPrice({ productName: 'Unapproved', price: 100, isApproved: false }));
    const estimates = buildPriceEstimates(catalog, [{ productName: 'Unapproved', quantity: 5 }]);
...
describe('Price Approval', () => {
  it('approves a price', () => {
    let catalog = createMarketPriceCatalog();
    const price = createMarketPrice({ productName: 'Test', isApproved: false, isVerified: false });
  ...
  it('rejects a price', () => {
    let catalog = createMarketPriceCatalog();
    const price = createMarketPrice({ productName: 'Test', isApproved: true, reliability: 'trusted' });
    catalog = addPrice(catalog, price);
...
describe('Formatting and Labels', () => {
  it('formats price estimate as text', () => {
    const estimate: any = {
      productName: 'Cement', brand: 'Lafarge',
      unitPrice: 7500, currency: 'NGN',
      quantity: 10, quantityUnit: 'bags',
      totalCost: 75000,
      source: 'vendor', freshnessLabel: 'This week',
      isVerified: true, confidence: 'high',
    };
