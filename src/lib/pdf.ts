...
export async function generateQuotationPDF(
  ...
  if (quotation.company_logo_url) {
    try {
      doc.addImage(quotation.company_logo_url, 'PNG', pageWidth - margin - 30, y, 30, 15);
    } catch {
  ...
  if (quotation.company_address) doc.text(quotation.company_address, margin, y);
  if (quotation.company_phone) doc.text(`Tel: ${quotation.company_phone}`, margin, y + 4);
  if (quotation.company_email) doc.text(`Email: ${quotation.company_email}`, margin, y + 8);
  ...
  doc.text(`Date: ${new Date(quotation.created_at).toLocaleDateString('en-GB')}`, pageWidth - margin, y, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  if (project.client_name) { doc.text(project.client_name, margin, y); y += 5; }
  if (project.client_phone) { doc.text(project.client_phone, margin, y); y += 5; }
  if (project.client_email) { doc.text(project.client_email, margin, y); y += 5; }
  if (project.client_address) { doc.text(project.client_address, margin, y); y += 5; }
  ...
  if (quotation.transport_cost > 0) rows.push(['Transport', quotation.transport_cost]);
  if (quotation.misc_cost > 0) rows.push(['Miscellaneous', quotation.misc_cost]);
  if (quotation.markup_amount > 0) rows.push([`Markup (${quotation.markup_percentage}%)`, quotation.markup_amount]);
  if (quotation.profit_amount > 0) rows.push([`Profit (${quotation.profit_percentage}%)`, quotation.profit_amount]);
  if (quotation.tax_amount > 0) rows.push([`Tax (${quotation.tax_percentage}%)`, quotation.tax_amount]);

  for (const [label, amount] of rows) {
    doc.text(label, margin + 2, y);
    doc.text(`${quotation.currency_symbol}${amount.toLocaleString('en-NG', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, pageWidth - margin - 2, y, { align: 'right' });
    y += 6;
  }
  ...
  doc.text(`${quotation.currency_symbol}${quotation.grand_total.toLocaleString('en-NG', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, pageWidth - margin - 2, y + 7, { align: 'right' });
  y += 18;
  ...
  if (quotation.terms_conditions) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    const termsLines = doc.splitTextToSize(quotation.terms_conditions, pageWidth - 2 * margin);
  ...
  doc.text(`Validity: ${quotation.validity_days} days from issue date`, margin, y);
  ...
  doc.text('Client Acceptance', pageWidth - margin - 70 + 35, y + 4, { align: 'center' });
...
export async function generateShoppingListPDF(
  ...
  for (const category of categories) {
    ...
    for (const item of categoryItems) {
      ...
      x += colWidths[2];
      doc.text(`${project.currency_symbol}${item.estimated_price.toLocaleString('en-NG')}`, x + 2, y);
      x += colWidths[3];
      doc.text(`${project.currency_symbol}${item.total_price.toLocaleString('en-NG')}`, x + 2, y);
    ...
    doc.text(`${project.currency_symbol}${categoryTotal.toLocaleString('en-NG')}`, margin + tableWidth, y, { align: 'right' });
    y += 8;
  ...
  doc.setFontSize(12);
  doc.text('GRAND TOTAL', margin + 2, y + 7);
  doc.text(`${project.currency_symbol}${grandTotal.toLocaleString('en-NG')}`, margin + tableWidth, y + 7, { align: 'right' });
