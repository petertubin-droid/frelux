...
export function checkGoogleCompliance(
  article: ArticleInput,
): GoogleComplianceReport {
  ...
  // E-E-A-T assessment
  const hasAuthor =
    result.rules.find((r) => r.rule === "author-attribution")?.passed ?? false;
  const hasContent =
    result.rules.find((r) => r.rule === "word-count-minimum")?.passed ?? false;
  const hasMetaDesc =
    result.rules.find((r) => r.rule === "meta-description-missing")?.passed ??
    true;
  const noPlaceholder =
    result.rules.find((r) => r.rule === "no-placeholder-content")?.passed ??
    false;
  const _hasConclusion =
    result.rules.find((r) => r.rule === "conclusion-section")?.passed ?? false;
  const hasSlug =
    result.rules.find((r) => r.rule === "slug-format")?.passed ?? false;

  const eeattAssessment = {
    experience:
      hasContent && noPlaceholder ? ("pass" as const) : ("fail" as const),
    expertise: hasAuthor && hasContent ? ("pass" as const) : ("fail" as const),
    authoritativeness:
      hasAuthor && hasSlug ? ("pass" as const) : ("fail" as const),
    trustworthiness:
      hasMetaDesc && noPlaceholder && hasSlug
        ? ("pass" as const)
        : ("fail" as const),
  };

  const eeattPassCount = Object.values(eeattAssessment).filter(
    (v) => v === "pass",
  ).length;
