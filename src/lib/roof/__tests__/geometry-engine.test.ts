...
import type { RoofPoint } from "../geometry-types";
...
describe("Roof Geometry: Polygon Area", () => {
  ...
  it("returns positive area regardless of winding order", () => {
    ...
    expect(polygonArea(clockwise)).toBeCloseTo(
      polygonArea(counterClockwise),
      2,
    );
  });
...
describe("Roof Geometry: Polygon Perimeter", () => {
  ...
  it("calculates perimeter of a triangle", () => {
    const points = [createPoint(0, 0), createPoint(30, 40), createPoint(30, 0)];
