 ...
 */
export function applyTrustEvent(fact: AiFact, event: TrustEvent, now = new Date().toISOString()): AiFact {
  const stamped = { ...fact, verifiedAt: now };
  switch (event.type) {
    case 'user_confirmed':
      return { ...stamped, trust: 'user_confirmed', origin: 'user_confirmed', confidence: 1 };
    case 'user_edited':
      return { ...stamped, trust: 'corrected', origin: 'user_confirmed', value: event.value, confidence: 1 };
    case 'user_rejected':
    ...
    case 'insufficient_evidence':
      return { ...stamped, trust: 'insufficient_evidence', confidence: 0, evidence: event.reason };
  }
}
...
export function canUseInCalculation(fact: AiFact): boolean {
  if (fact.trust === 'rejected' || fact.trust === 'insufficient_evidence' || fact.trust === 'unknown') {
    return false;
  }
...
export function toMeasurementVerificationState(
  ...
  if (fact.trust === 'user_confirmed' || fact.trust === 'corrected') return 'user_verified';
...
export function fromExtractionStatus(
  ...
  switch (status) {
    ...
    case 'user_edited': return 'corrected';
    case 'rejected': return 'rejected';
    case 'requires_confirmation': return 'needs_confirmation';
...
export function trustBadge(fact: AiFact): string {
  switch (fact.trust) {
    ...
    case 'system_verified': return fact.origin === 'engine_calculation' ? 'Calculated by FRELUX engine' : 'Verified';
    case 'corrected': return 'Corrected by you';
    case 'rejected': return 'Rejected';
    case 'needs_confirmation': return 'Needs your confirmation';
    case 'insufficient_evidence': return 'Insufficient evidence';
    case 'unknown': return 'Unknown origin';
    default: return 'Detected — assumption';
  }
}
