...
registerEngine({
  ...
  async run(rawInput) {
    ...
    const input = rawInput as {
      length: number; width: number; wallHeight: number;
      projectType?: 'room' | 'whole_house' | 'exterior';
      unit?: 'meters' | 'feet';
      doors?: number; windows?: number;
      coats?: number; paintType?: string; includeCeiling?: boolean; wasteMargin?: number;
    };
    const invalid = requireFiniteNumbers({
      length: input.length, width: input.width, wallHeight: input.wallHeight,
      doors: input.doors ?? 0, windows: input.windows ?? 0, coats: input.coats ?? 2,
      wasteMargin: input.wasteMargin ?? 10,
    });
    if (invalid) {
      return { ok: false, engine: 'painting_project', calculatedAt: new Date().toISOString(), quantities: [], costs: null, raw: null, error: invalid };
    }
    ...
    const quantities: EngineQuantityLine[] = [
      ...
      quantityLine('Paint required (with waste)', result.adjustedLiters, 'litres'),
      quantityLine('Total recommended litres', result.totalRecommendedLiters, 'litres'),
    ];
    for (const container of result.recommendedContainers ?? []) {
      quantities.push(quantityLine(`${container.size} litre containers`, container.count, 'containers'));
    }
...
registerEngine({
  ...
  async run(rawInput) {
    ...
    const input = rawInput as {
      surfaceType: 'floor' | 'wall'; method: 'traditional' | 'adhesive';
      length: number; width: number; height?: number;
      unit?: 'meters' | 'feet';
      tileWidthMm: number; tileHeightMm: number; tilesPerBox: number;
    ...
    const invalid = requireFiniteNumbers({
      ...
      tileWidthMm: input.tileWidthMm, tileHeightMm: input.tileHeightMm,
      tilesPerBox: input.tilesPerBox, tilePricePerBox: input.tilePricePerBox,
    });
    if (invalid) {
      return { ok: false, engine: 'tile_estimate', calculatedAt: new Date().toISOString(), quantities: [], costs: null, raw: null, error: invalid };
    }
    ...
    const result = calculateTile({
      ...
      labourRatePerSqm: 0,
      unit: (input.unit ?? 'meters') as never,
    } as never, [] as never, currency, symbol);
    ...
    if (result.tileCost > 0) costLines.push({ label: 'Tiles', amount: result.tileCost });
    if (result.adhesiveCost > 0) costLines.push({ label: 'Adhesive', amount: result.adhesiveCost });
    if (result.cementCost > 0) costLines.push({ label: 'Cement', amount: result.cementCost });
    if (result.groutCost > 0) costLines.push({ label: 'Grout', amount: result.groutCost });
    const costs: EngineCostSummary | null = costLines.length
      ? { total: costLines.reduce((s, l) => s + l.amount, 0), currency, lines: costLines, regionalDataAvailable: false }
      : null;
    return {
      ok: true,
...
registerEngine({
  ...
  async run(rawInput) {
    ...
    const input = rawInput as {
      roomLength: number; roomWidth: number;
      unit?: 'meters' | 'feet'; wasteMargin?: number;
      workflow?: 'nigeria' | 'international';
      includeDecorative?: boolean; includeOptional?: boolean;
      currency?: string; currencySymbol?: string;
      materials?: Array<Record<string, unknown>>;
    };
    const invalid = requireFiniteNumbers({
      roomLength: input.roomLength, roomWidth: input.roomWidth,
      wasteMargin: input.wasteMargin ?? 10,
    });
    if (invalid) {
      return { ok: false, engine: 'pop_ceiling', calculatedAt: new Date().toISOString(), quantities: [], costs: null, raw: null, error: invalid };
    }
    ...
    if (!materials || materials.length === 0) {
      return {
        ...
        costs: null,
        raw: null,
        error: 'POP materials data is unavailable right now — use the POP Ceiling Calculator directly for this estimate.',
      };
    }
    ...
    } as never, materials as never, currency, symbol);
...
registerEngine({
  ...
  async run(rawInput) {
    ...
    const invalid = requireFiniteNumbers({ areaM2: input.areaM2, coats: input.coats ?? 0 });
    if (invalid) {
      return { ok: false, engine: 'screeding_system', calculatedAt: new Date().toISOString(), quantities: [], costs: null, raw: null, error: invalid };
    }

    let config = (input.config as Awaited<ReturnType<typeof dbToSystemConfig>> | undefined) ?? undefined;
    if (!config) {
      const { fetchScreedingSystemConfig } = await import('@/lib/queries');
      const { data, error: cfgError } = await fetchScreedingSystemConfig(input.systemType ?? 'white_cement_paint');
      if (cfgError || !data) {
        return {
          ok: false,
          ...
          costs: null,
          raw: null,
          error: 'Screeding configuration is unavailable right now — use the Screeding Cost Estimator directly for this estimate.',
        };
      }
      config = dbToSystemConfig(data);
    }

    const result = calculateScreedingSystem(input.areaM2, config, input.coats ?? config.defaultCoats);

    const quantities: EngineQuantityLine[] = [
      quantityLine('Net screeding area', result.netScreedingArea, 'm²'),
    ];
    const costLines: Array<{ label: string; amount: number }> = [];
    const addBreakdown = (label: string, breakdown: { quantity?: number; unit?: string; cost?: number | null } | null) => {
      if (!breakdown) return;
      quantities.push(quantityLine(label, Number(breakdown.quantity ?? 0), String(breakdown.unit ?? 'units')));
      if (typeof breakdown.cost === 'number' && breakdown.cost > 0) {
        costLines.push({ label, amount: breakdown.cost });
      }
    };
    ...
    const costs: EngineCostSummary | null = costLines.length
      ? { total: result.materialCost ?? costLines.reduce((s, l) => s + l.amount, 0), currency: result.currency, lines: costLines, regionalDataAvailable: false }
      : null;
    return {
      ok: true,
      engine: 'screeding_system',
