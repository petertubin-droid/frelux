...
describe("vocabulary interop", () => {
  it("maps to the pre-existing measurement verification states", () => {
    expect(
      toMeasurementVerificationState(
        createFact(fact({ origin: "user_input" })),
      ),
    ).toBe("manual_input");
    expect(
      toMeasurementVerificationState(
        createFact(fact({ origin: "engine_calculation" })),
      ),
    ).toBe("calculated");
    expect(
      toMeasurementVerificationState(
        createFact(fact({ origin: "market_data" })),
      ),
    ).toBe("market_price_verified");
    expect(
      toMeasurementVerificationState({
        ...fact(),
    ...
    expect(toMeasurementVerificationState(createFact(fact()))).toBe(
  ...
  it("maps from the pre-existing extraction statuses", () => {
    ...
    expect(fromExtractionStatus("requires_confirmation")).toBe(
      "needs_confirmation",
    );
  ...
  it("never shows a fake verified badge", () => {
    ...
    expect(
      trustBadge(createFact(fact({ origin: "engine_calculation" }))),
    ).toContain("Calculated by FRELUX");
  });
});
